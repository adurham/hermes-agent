            ...
            try:
                if env_type == "local":
                    proc_session = process_registry.spawn_local(
                        ...
                        # while the subagent is still alive. Falling back to ""
                        # (rather than effective_task_id) here is deliberate and
                        # harmless: spawn_local/spawn_via_env already do
                        # `owner_task_id or task_id` internally, so an empty
                        # string here resolves to the same effective_task_id
                        # fallback either way.
                    ...
                    proc_session = process_registry.spawn_via_env(
                        ...
                        task_id=effective_task_id,
                        session_key=session_key,
                        # See spawn_local's owner_task_id comment above: RAW
                        # caller task_id, not effective_task_id -- registry
                        # falls back to task_id internally either way.
                        owner_task_id=task_id or "",
                    )
