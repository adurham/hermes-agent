...
def resolve_send_target(
    ...
    if platform_name not in _KNOWN_PARSER_PLATFORMS and (
        entry is None or entry.parse_target_ref_fn is None
    ):
        # Nothing in the channel directory matched, AND the platform has no
        # bespoke ``_parse_target_ref`` rule of its own (plugin platforms
        # like A2A whose channel ids are opaque context ids from
        ...
        # works, instead of falling through to a misleading "No home
        # channel set" / generic resolution error (issue #78396).
        error = _validate(target_ref)
        return (None, None, error) if error else (target_ref, None, None)
