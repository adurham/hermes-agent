...
class ProcessRegistry(ProcessCheckpointMixin):
    ...
    def drain_notifications(
        ...
        while not self.completion_queue.empty():
            ...
            #
            # When delegation.surface_child_process_notifications=true the
            # operator has explicitly opted INTO seeing child notifications
            # in the parent immediately — the hold's "wait for the owner to
            # exit so the parent isn't interrupted mid-task" rationale is
            # moot, and the suppression gate below also skips such events.
            # Apply the same bypass here so the two gates agree on precedence
            # (surface_child=true ⇒ deliver now, no hold, no drop). Resolve
            # the flag lazily (first sa- event only) so this reads config at
            # most once per drain, matching the suppression gate below.
            if (
                evt.get("type") == "completion"
                and (evt.get("owner_task_id") or evt.get("task_id") or "").startswith("sa-")
            ):
                if surface_child is None:
                    surface_child = self._surface_child_process_notifications()
            if surface_child is not True:
            ...
            if not is_async_delegation and _evt_task_id.startswith("sa-"):
                # An event that was HELD by the liveness gate (owner alive at
                # first sight, then exited / TTL expired) is released to the
                # parent via the documented orphan path and must be delivered
                # with attribution, NOT re-dropped here. The suppression gate
                # only applies to events never gated — i.e. child completions
                # whose owner was already gone on arrival (true leaks), which
                # never acquired a hold stamp.
                if _EVENT_HELD_AT_KEY not in evt:
                    if surface_child is None:
                        surface_child = self._surface_child_process_notifications()
    ...
    def wait(self, session_id: str, timeout: int = None) -> dict:
        ...
        ``timeout`` defaults to TERMINAL_TIMEOUT and is clamped to
        ``terminal.wait_max_timeout`` (TERMINAL_WAIT_MAX_TIMEOUT, default 3600s).
        Returns a dict with status exited|timeout|interrupted|not_found|error and an
        output snapshot."""
        ...
        output_tail = _output_tail(session, 1000)
...
# --- fork-only completion helper (the generic renderers, including
# _format_async_delegation, moved to tools/process_registry_notifications.py
# in the Sep 2026 decomposition; this one stays here with its consumers/tests).
...
def event_owner_still_running(evt: dict) -> bool:
    ...
    """
    # Read the RAW spawning id first (owner_task_id); the event's task_id is
    # the COLLAPSED container key ("default"/session key) for every subagent
    # on local backend. Mirrors the idiom in _delegation_attribution_line
    # and the drain suppression gate.
    task_id = evt.get("owner_task_id") or evt.get("task_id")
