...
def _run_search_single(query: str, limit: int) -> dict:
    ...
    try:
        ...
        if _rescue_eligible(provider):
            return _rescue_search(provider.name, str(exc), query, limit)
        raise
    if not response_data.get("success") and _rescue_eligible(provider):
        # One-shot keyless rescue: THIS call rides the free-tier ring; the
        # next call attempts the chosen backend again.
        return _rescue_search(
            provider.name, str(response_data.get("error", "")), query, limit,
        )
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if chain:
            ...
            # Single-provider dispatch. Upstream's backend-selection errors and
            # keyless-rescue logic live INSIDE _run_search_single so the fork's
            # chain-vs-single seam (tests/tools/test_web_search_chain.py) stays
            # monkeypatchable.
