...
def _run_search_single(query: str, limit: int) -> dict:
    ...
    Upstream's backend-selection diagnostics (``selection_error`` for a
    configured-but-unregistered backend, ``_disabled_web_plugin_for`` for a
    disabled bundled plugin) and its one-shot keyless rescue are folded in
    here rather than at the web_search_tool call site, so the fork's
    chain-vs-single dispatch seam stays a single monkeypatchable function.
    """
    ...
    if provider is None:
        # A bundled web plugin the user explicitly disabled looks identical to
        # "no provider" here — point at the real cause.
        disabled_key = _disabled_web_plugin_for(capability="search")
        if disabled_key:
            _vendor = disabled_key.split("/", 1)[-1]
            return {
    ...
    # Sits after every safety/config check and directly around the paid
    # vendor call. Identical queries within the TTL (subagent fan-outs,
    # repeat lookups) are served from memory; concurrent identical queries
    # share one request via the flight lock. The provider is asked for the
    # BUCKETED count (10/20/50/100) so near-identical limits share an entry;
    # the caller's requested count is sliced out below. Only successful
    # responses cache. Ported into this seam (rather than the
    # web_search_tool call site) so the fork's chain-vs-single dispatch
    # boundary (tests/tools/test_web_search_chain.py) stays monkeypatchable
    # and the rescue/backend-selection diagnostics above still apply.
    ...
    def _paid_search() -> tuple[dict, bool]:
        ...
        _rescued = False
        logger.info("Web search via %s: '%s' (limit: %d)", provider.name, query, _fetch_limit)
        try:
            _resp = provider.search(query, _fetch_limit)
        except Exception as exc:  # noqa: BLE001 — candidate for rescue
            if _rescue_eligible(provider):
                _rescued = True
                _resp = _rescue_search(provider.name, str(exc), query, _fetch_limit)
            else:
                raise
        else:
            if not _resp.get("success") and _rescue_eligible(provider):
                ...
                _resp = _rescue_search(
                    provider.name, str(_resp.get("error", "")), query, _fetch_limit,
                )
    ...
    if response_data is None:
        with _search_memo.flight_lock(provider.name, query, limit):
            # Re-check inside the lock: a concurrent identical call may
            # have stored while this one waited.
            response_data = _search_memo.lookup(provider.name, query, limit)
            if response_data is None:
                response_data, _was_rescued = _paid_search()
                # Never cache a rescue-served response: it came from a ring
                # vendor, not the chosen backend (wrong key), and caching it
                # would make the one-shot rescue sticky for this query for a
                # whole TTL — the next call must attempt the chosen backend
                # again.
                if not _was_rescued:
                    _search_memo.store(provider.name, query, limit, response_data)
    return _slice_search_response(response_data, limit)
...
def web_search_tool(query: str, limit: int = 5) -> str:
    ...
    try:
        ...
        if chain:
            # FORK: explicit failover chain — walk providers in order, fall
            # through on 429 / rate-limit / any failure. Returns the first
            # successful response, or the last error if all providers fail.
            response_data = _run_search_chain(chain, query, limit)
        else:
            # Single-provider dispatch. Upstream's backend-selection errors,
            # keyless-rescue logic, and TTL result cache (tools/web_result_cache.py)
            # all live INSIDE _run_search_single so the fork's chain-vs-single
            # seam (tests/tools/test_web_search_chain.py) stays monkeypatchable.
...
def check_web_api_key() -> bool:
    ...
    try:
        ...
        # Upstream's stricter readiness predicate (keyed OR keyless-capable),
        # but kept as a fall-THROUGH rather than upstream's unconditional
        # `return`: the fork's Anthropic-native web-search path below is a
        # legitimate second source of availability, and returning False here
