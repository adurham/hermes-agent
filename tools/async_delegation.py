...
def active_task_count() -> int:
    ...
    with _records_lock:
        return sum(
            ...
            for r in _records.values() if r.get("status") in {"running", "stalling", "finalizing"})
...
def _matches_session_selectors(
    ...
    """Whether ``record`` is owned by the session identified by any non-empty selector.

    The single-record form of ``_session_records``' ownership rule (same any-of
    semantics over the same three fields), shared with ``interrupt_by_id``. An
    empty selector never claims a record; with all three empty this returns
    False, so callers must decide separately whether to scope at all.
    """
    return any(
        str(record.get(field) or "") == wanted
        for field, wanted in (
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    evt = {
        ...
        **({"unprocessed_messages": result["unprocessed_messages"]} if "unprocessed_messages" in result else {})}
    try:
