...
def _resolve_checkpoint_base() -> Path:
    ...
    Fork note (re-homed onto upstream's helper, was ``_default_base()``): binding the base at
    import time is what let the 2026-07-12 test run clear the production store — a later env
    change (test monkeypatch, profile switch) was silently ignored by default-arg callers."""
