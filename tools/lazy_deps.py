...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "provider.anthropic": ("anthropic==0.100.0",),  # CVE-2026-34450, CVE-2026-34452 fixed in 0.87.0+; kept in sync with pyproject.toml's exclude-newer-package pin
...
def _refresh_features(features: list[str], *, prompt: bool, restoring: bool) -> dict[str, str]:
    ...
    for feature in features:
        ...
        try:
            ...
            skip = ("lazy installs disabled" in str(e)
                    or "is blocked by security.blocked_features" in str(e)
                    or "declined" in str(e) or e.reason.startswith("unsupported "))
