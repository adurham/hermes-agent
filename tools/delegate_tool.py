            ...
            with SwarmBoard.maybe_start(parent_agent, n_tasks) as _swarm_board:
                ...
                with DaemonThreadPoolExecutor(max_workers=max_children) as executor:
                    ...
                    def _submit_child(idx_t_child):
                        ...
                        # Propagate the parent's contextvars into the worker
                        # thread so per-context state (approval scopes, task
                        # ids, ...) is visible to the child run (upstream).
                        _child_context = contextvars.copy_context()
