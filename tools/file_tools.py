...
def _handle_read_file(args, **kw):
    ...
    return read_file_tool(path=raw_path, offset=args.get("offset", 1), limit=args.get("limit", DEFAULT_READ_LIMIT), task_id=tid)
...
def _handle_search_files(args, **kw):
    ...
    if path is None or (isinstance(path, str) and not path.strip()):
        path = "."
    return search_tool(
        pattern=raw_pattern, target=target, path=path,
...
registry.register(name="write_file", toolset="file", schema=WRITE_FILE_SCHEMA, handler=_handle_write_file, check_fn=_check_file_reqs, emoji="✍", max_result_size_chars=100_000)
