...
class ToolSearchConfig:
    ...
    # FORK: opt normally-core toolsets / tools into lazy loading. Empty by default
    # (upstream base behavior: only MCP + non-core plugin tools defer).
    #   defer_toolsets  — registry toolset names (e.g. "browser", "homeassistant") whose
    #                     tools defer behind the tool_search/describe/call bridge even
    #                     though they are listed in toolsets._HERMES_CORE_TOOLS.
    #   defer_tools     — individual tool names to force-defer (config key "defer_tools"),
    #                     ADDITIVE on top of the curated set below. NOTE: upstream's own
    #                     wholesale-replacement list (config key "defer") is a SEPARATE
    #                     field, ``defer_override`` — do not merge the two, they answer
    #                     different questions and both are asserted in tests.
    #   keep_eager_tools— individual tool names that must NEVER defer, even when their
    #                     toolset is in defer_toolsets or the curated set. Wins over
    #                     everything (e.g. keep "delegate_task" eager while deferring a
    #                     toolset sibling).
    defer_toolsets: frozenset = frozenset()
    defer_tools: frozenset = frozenset()
    keep_eager_tools: frozenset = frozenset()
    # Catalog listing ("skills-style" progressive disclosure): when active, a grouped
    # name + short-description manifest of every deferred tool is embedded in the
    # tool_search bridge description, so capabilities stay DISCOVERABLE while full
    # schemas stay deferred. "auto"/"on" = embed the manifest when it fits the listing
    # budget (falls back to names-only, then to none = bare bridge); "off" = bare bridge.
    ...
    # UPSTREAM (config key "defer"): None = use the curated _DEFAULT_DEFERRED_TOOLS set;
    # an explicit list REPLACES it wholesale ([] = defer no core tools).
    defer_override: Optional[frozenset] = None

    @property
    def effective_defer_tools(self) -> frozenset:
        """Names deferred by list membership: upstream's curated/overridden set, plus the
        fork's additive ``defer_tools``, minus ``keep_eager_tools`` (which always wins).

        Subtracting keep_eager here — not only in ``is_deferrable_tool_name`` — keeps the
        guarantee intact for the call sites that pass this frozenset instead of the whole
        config object."""
        base = (_DEFAULT_DEFERRED_TOOLS if self.defer_override is None
                else self.defer_override)
        return (base | self.defer_tools) - self.keep_eager_tools
    ...
    def from_raw(cls, raw: Any) -> "ToolSearchConfig":
        ...
        return cls(
            ...
            defer_override=(frozenset(str(n).strip() for n in defer_raw if str(n).strip())
...
def is_deferrable_tool_name(
        name: str,
        defer_tools: Any = None) -> bool:  # frozenset | ToolSearchConfig | None
    """Return True if a tool with this name is *eligible* for deferral.

    Base rule (upstream): a tool is deferrable iff it is named in the effective defer
    set, OR registered with an MCP toolset prefix, OR neither core nor a session-gated
    GUI surface toolset (i.e. a plugin tool). Bridge names never defer.

    ``defer_tools`` accepts EITHER shape on the same positional slot:
      * a ``frozenset`` of names — upstream's internal call convention
    ...
    FORK override (precedence, highest first):
      ...
      2. ``keep_eager_tools`` — name listed here never defers, full stop. Lets you keep
         one tool eager while deferring its toolset siblings (e.g. keep ``delegate_task``
         while deferring another toolset tool). Also applied inside
         ``effective_defer_tools`` so the frozenset call convention honors it too.
      3. the effective defer set (curated/overridden + ``defer_tools``) — always defers,
         even if core.
      4. ``defer_toolsets`` — tool whose registry toolset is listed here always defers,
         even if core (e.g. defer the whole ``browser`` toolset). This is what lets
         normally-always-loaded toolsets become lazy-loaded behind the bridge. Requires
         the config object; unreachable via the bare-frozenset convention.
      5. Fall through to the upstream base rule.
    """
    if name in BRIDGE_TOOL_NAMES:
        return False
    config = None
    if isinstance(defer_tools, ToolSearchConfig):
        config = defer_tools
    ...
    if config is not None and name in config.keep_eager_tools:
        return False
    # (3) defer-set membership (curated + explicit per-tool) overrides core status.
    if name in names:
        return True

    # (4) toolset-level force-defer (overrides core status).
    if config is not None and config.defer_toolsets:
        if _registry_toolset(name) in config.defer_toolsets:
            return True
...
def classify_tools(tool_defs: List[Dict[str, Any]],
                   defer_tools: Any = None,  # frozenset | ToolSearchConfig | None
                   ) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    """Split a tool-defs list into (visible, deferrable); bridge tools are dropped (re-added
    after classification). ``visible`` retains every tool that must stay in the model-facing
    array, including any tool we cannot classify.

    ``defer_tools`` is resolved ONCE here and threaded into each
    ``is_deferrable_tool_name`` call (same union shape) so the fork's
    defer_toolsets/defer_tools/keep_eager_tools lists are honored without a per-tool
    config reload."""
    if defer_tools is None:
        defer_tools = load_config()
...
def should_activate(config: ToolSearchConfig, deferrable_tokens: int,
    ...
    ``"off"`` never activates (a global kill switch stays absolute). ``"on"`` activates
    whenever at least one deferrable tool exists (no point swapping a no-op).

    FORK: ``"auto"`` keeps a thresholded gate — it activates when the deferrable schemas
    would consume ``threshold_pct`` of context or more. Upstream collapsed ``"auto"`` into
    an alias of ``"on"``; the fork keeps the gate so a handful of deferred tools does not
    buy every small session the bridge indirection. Explicit ``defer_toolsets`` /
    ``defer_tools`` lists express direct user intent and override the threshold.

    ``connections_granted`` (upstream): remote connector accounts are reachable through
    the bridge even with nothing locally deferrable, so they activate it on their own.
    ``context_length`` is retained for caller compatibility."""
...
class AssemblyResult:
    ...
    # FORK: True when this assembly activated (or stayed activated) purely because
    # sticky_active=True was passed in, i.e. the live deferrable-token total no longer
    # clears should_activate() on its own. Observability only — lets callers and logs
    # distinguish "activated because sticky" from "activated because still over threshold".
...
def assemble_tool_defs(tool_defs: List[Dict[str, Any]], *, context_length: Optional[int] = None,
                       ...
                       sticky_active: bool = False) -> AssemblyResult:
    ...
    ``sticky_active`` (FORK): the *catalog* (which tools are deferrable and their live
    schemas) is always rebuilt fresh from ``tool_defs`` on every call — this function never
    caches that. But the boolean activate/deactivate decision is NOT safe to recompute from
    scratch every call within one ongoing conversation: ``classify_tools`` walks the live,
    global ``tools/registry.py`` singleton, so an unrelated MCP reconnect or a concurrent
    subagent loading tools can shift the deferrable-token total across the
    ``threshold_pct`` boundary between two API calls of the *same* conversation. When that
    flips activation from on to off, the bridge tool names disappear from the wire tools
    array for that turn, and Anthropic's API rejects any previous-turn tool_use block
    referencing them — which ``_strip_unknown_tool_blocks`` then rewrites into inert text
    breadcrumbs, corrupting tool-call history mid-conversation. Passing
    ``sticky_active=True`` (the caller's per-conversation "have we ever activated" flag)
    forces activation to stay on once it was ever on, without touching what is actually in
    the catalog. ONE-WAY latch: off->on still requires clearing the normal threshold; only
    on->off is suppressed. Callers thread the flag from the agent object — see
    model_tools.py / agent_init.py / tools/mcp_tool.py / acp_adapter/server.py."""
    config = config or load_config()
    incoming = [td for td, name in zip(tool_defs, _tool_def_names(tool_defs))
                if name not in BRIDGE_TOOL_NAMES]
    # Pass the whole config, not just ``effective_defer_tools``: the fork's
    # defer_toolsets rule needs the config object to resolve a tool's registry toolset.
    visible, deferrable = classify_tools(incoming, config)
    connections_granted = connections_in_scope(incoming)
    if not deferrable:
        # sticky never conjures a bridge out of thin air — nothing to hide behind it.
    ...
    logger.info(
        "tool_search %s (tier %d): %d core/visible tools kept, %d deferred "
        "(~%d tokens), listing %s (budget ~%d tokens)",
        ("stays activated (sticky — live total now below threshold, but this conversation "
         "already activated tool_search, so bridge tools stay present to avoid corrupting "
         "tool-call history; see assemble_tool_defs sticky_active docs)"
         if sticky_forced else "activated"),
    ...
    return AssemblyResult(
        ...
        sticky_forced=sticky_forced, tier=tier, listing_form=listing_form)
...
def tool_defs_show_bridge(tool_defs: Optional[List[Dict[str, Any]]]) -> bool:
    """True if any bridge tool name is present in a tool-defs list.

    FORK. Cheap way for callers (agent_init, refresh_agent_mcp_tools, the ACP server) to
    detect whether an assembled tools array activated progressive disclosure, without
    threading AssemblyResult through every call site — ``get_tool_definitions`` returns a
    plain list, not an AssemblyResult. Used to set/update the caller's sticky
    "ever activated" flag (``agent._tool_search_ever_activated``)."""
    if not tool_defs:
        return False
    return any(name in BRIDGE_TOOL_NAMES for name in _tool_def_names(tool_defs))
