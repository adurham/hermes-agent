...
def is_deferrable_tool_name(name: str, config: Optional["ToolSearchConfig"] = None) -> bool:
    ...
    MCP toolset prefix OR it is neither in ``_HERMES_CORE_TOOLS`` nor a
    session-gated GUI surface toolset. Core and direct surface tools are
...
def assemble_tool_defs(
    ...
    *,
    ...
    if sticky_forced:
        logger.info(
            "tool_search stays activated (sticky, tier %d): %d core/visible tools kept, "
            ...
            "history (see assemble_tool_defs sticky_active docs). listing %s (budget ~%d tokens)",
            tier, len(visible), len(deferrable), deferrable_tokens, threshold_tokens,
            listing_form, listing_budget,
        )
    else:
        logger.info(
            "tool_search activated (tier %d): %d core/visible tools kept, %d deferred "
            "(~%d tokens, threshold ~%d), listing %s (budget ~%d tokens)",
            tier, len(visible), len(deferrable), deferrable_tokens, threshold_tokens,
            listing_form, listing_budget,
        )
