...
class ToolEntry:
    ...
    # Optional predicate: return True when the tool owns its OWN completion bound and the
    # generic per-call executor deadline must NOT be applied. Set for tools whose legitimate
    # runtime is the runtime of work they supervise (delegate_task blocking spawns) rather than
    # of their own code. The executor still runs them on a worker with interrupt polling — "no
    # deadline" never means "no interrupt checks" — it only skips the synthetic timeout+abandon
    # path that would detach a still-running supervisor from its result.
    # Signature: fn(args: dict, agent: Any) -> bool. Exceptions fail CLOSED (deadline applies)
    # so a buggy predicate can never remove a bound silently.
...
class ToolRegistry:
    ...
    def register(
        ...
        dynamic_schema_overrides: Callable = None, owns_own_deadline: Optional[Callable] = None,
