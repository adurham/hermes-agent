...
# ---------------------------------------------------------------------------
#
# v2026.8.3 upstream sync note (2026-08-04): this subsystem (_cache_path_for,
# _hash_server_config, _load_cached_spec, _make_cached_server_shell,
# _save_cached_spec, _ensure_server_connected, _spawn_background_refresh) is
# fork's own default-on cache-fast-path and DELIBERATELY COEXISTS with
# upstream's opt-in, `lazy: true`-gated mcp_schema_cache.py mechanism
# (#56832). Upstream's lazy path is consulted first in register_mcp_servers;
# any remaining eager server with a valid on-disk cache entry takes this
# fast path. _ensure_server_connected also remains the dead-session
# lazy-spawn recovery path for non-recycled-stdio servers in
# _make_tool_handler / _resolve_live_server (see FORK.md, 2026-07-27
...
def _resolve_live_server(server_name: str, tool_label: str):
    ...
    """
    # Routes through _get_connected_server_for_call so lazy (`lazy: true`,
    # schema-cache registered, #56832) servers first-use connect and
    # recycled-stdio servers lazy-reconnect on resource/prompt calls too,
    # not just raw tool calls.
    ...
    if server.session is None:
        ...
        # alive, just the session was torn down on idle timeout). That
        # reconnect was already attempted inside
        # _get_connected_server_for_call; still-down means fail cleanly.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        # Resolve the live server. This handles the upstream lazy path
        # (`lazy: true` servers registered from the schema cache connect on
        # first use, #56832) and recycled-stdio lazy reconnects.
        server = _get_connected_server_for_call(server_name)
        ...
        if server.session is None:
            ...
            # (already attempted inside _get_connected_server_for_call);
            # if the session is still down, fail cleanly rather than doing
            # a full config-backed respawn of a process that's still alive.
            if getattr(server, "_is_recycled_stdio", lambda: False)():
                _bump_server_error(server_name)
                return tool_error(f"MCP server '{server_name}' is not connected")
            # Cache shell or dead/parked transport: lazy-spawn the real
            # session from config (fork's _ensure_server_connected —
            # see FORK.md 2026-07-27; this is the non-recycled dead-session
            # recovery contract the circuit-breaker tests lock in).
        ...
        async def _call():
            ...
            async with server._rpc_lock, _track_inflight_rpc(
                ...
                try:
                    ...
                    # Call the watcher factory exactly once — calling it a
                    # second time (once here to probe isawaitable, again
                    # below to actually schedule it) leaked a coroutine that
                    # was never awaited (RuntimeWarning on every tool call).
                    ...
                    if not _watch_ok:
                        ...
                        # semantics. Close the unused watcher coroutine (if
                        # any) so it doesn't leak a "never awaited" warning.
                        ...
                        # tool timeout.
                        assert _watch_coro is not None  # guaranteed by _watch_ok
...
def _make_check_fn(server_name: str):
    ...
    def _check() -> bool:
        ...
        # is a recycled-stdio server awaiting lazy reconnect, or is registered
        # as a lazy server awaiting first-use connect (#56832). False when
        # the server doesn't exist or has been parked (reconnect budget
        # exhausted, tools deregistered).
        if server is None:
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    # ------------------------------------------------------------------
    # Startup-cache fast path (fork): any remaining server with a valid
    # on-disk cache gets its tools registered immediately (no subprocess,
    # no network) and is removed from ``new_servers`` so the live-discovery
    # loop only deals with the genuine cache-miss set. The real session is
    # lazy-spawned on first tool call (_ensure_server_connected) and a
    # daemon thread refreshes the cache in the background.
    # ------------------------------------------------------------------
    ...
    if not new_servers:
        # Background refresh keeps the cache fresh for the next session
        # even when every server was served from cache.
        if cache_hit_servers:
            _ensure_mcp_loop()
    ...
    new_tool_count += lazy_registered + cache_hit_tool_count
    connected_count = len(connected) + lazy_server_count + len(cache_hit_servers)
...
def get_mcp_status() -> List[dict]:
    ...
    report ``connected=True`` with the cached tool count and an extra
    ``lazy_spawn_pending`` flag — from the user's perspective those tools
    ARE available; the connection is just deferred until the model actually
    calls one.
    """
    result: List[dict] = []
