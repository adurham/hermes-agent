...
def _preamble(
    evt: dict, title: str, intro: str, completed_at: float, *, with_goal: bool,
    model_label: "str | None" = None,
) -> "list[str]":
    """Shared preamble: title, intro, blank, dispatch time, [goal], context/toolsets, role+model.

    ``model_label``, when given, overrides the raw ``evt['model']`` dispatch default in the
    final line. Callers with a live child result (single-task or batch) resolve this via
    ``_result_model_label``/the batch per-task-model logic so a task that silently failed over
    to a different model renders with the fork's ⚠ marker instead of reporting the model it was
    merely DISPATCHED with — see ``_single_completion_model_label``/``_batch_model_label``.
    """
    lines = [title, intro, ""]
...
def _single_completion_model_label(evt: dict) -> "str | None":
    """The single-task completion's actual (post-failover) model, when known.

    ``evt`` for a single (non-batch) async_delegation completion carries the same
    live-resolved ``model``/``fallback_active``/``primary_model`` keys a batch result entry
    does (delegate_tool's result build resolves them off the child at completion) — reuse
    ``_result_model_label`` so a silent provider failover renders with its ⚠ marker here too,
    not just in the batch path.
    """
    if not evt.get("model"):
        return None
    from tools.process_registry import _result_model_label
    return _result_model_label(evt)


def _batch_task_models(results: list) -> "dict[object, str]":
    """Each task's actual (post-failover) model, keyed by ``task_index``, when known.

    Shared by ``_batch_model_label`` (collapses to the header line) and the per-task
    wall (names each task's model inline when the batch is heterogeneous).
    """
    from tools.process_registry import _result_model_label
    return {
...
def _batch_model_label(task_models: "dict[object, str]", default_model: "str | None") -> "str | None":
    """Per-task models, when known, override the batch's dispatch-default ``model``.

    The batch-level ``model`` is only the dispatch default (``delegation.by_provider.<p>.model``
    / ``delegation.model``); a per-task pin (explicit ``model``, an ``agent_type`` role map, or
    auto-route) makes every child's ACTUAL model invisible here otherwise, and the header would
    confidently misreport what every task ran on. Each result also carries its own failover
    state, so a task that silently switched providers renders with the ⚠ marker via
    ``_result_model_label`` instead of being reported as if it ran on its configured model.
    Returns None when every task matches the batch default (the plain ``evt['model']`` line is
    already correct and this stays a no-op), a single label when every task's ACTUAL model
    agrees, or an explicit \"batch default; per-task varies\" note when they don't.
    """
    distinct = set(task_models.values())
    if not distinct or distinct == {default_model}:
        return None
    return next(iter(distinct)) if len(distinct) == 1 else f"{default_model} (batch default; per-task varies)"
...
def _format_batch_delegation(evt: dict, deleg_id: str, completed_at: float) -> str:
    ...
    task_models = _batch_task_models(results)
    default_model = evt.get("model")
    lines = _preamble(
        evt,
        ...
        model_label=_batch_model_label(task_models, default_model))
    ...
    # Heterogeneous batch: name each task's model inline too, so the mix is
    # auditable at a glance rather than only summarized in the header.
    distinct_task_models = set(task_models.values())
    for r in sorted(results, key=lambda x: x.get("task_index", 0)):
        ...
        header = (f"--- {icon} TASK {idx + 1}/{n}" + (f": {r_goal}" if r_goal else "") + f"  (status={r_status}"
                  ...
                  + (f", model={task_models[idx]}" if len(distinct_task_models) > 1 and task_models.get(idx) else "")
...
def _format_async_delegation(evt: dict) -> str:
    ...
    lines = _preamble(
        ...
        model_label=_single_completion_model_label(evt))
