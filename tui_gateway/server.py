...
def _get_usage(agent) -> dict:
    ...
    with contextlib.suppress(Exception):  # a status-bar readout must never break usage reporting
        ...
        if _n := min(len(_lhist), len(_ohist)):
            ...
            # FORK: `_api_latency_history` is DECODE-ONLY (full duration minus TTFT), so avg_tps is
            # true decode throughput; full-wall latency lives in `_api_full_latency_history`.
            _avg_lat = (sum(_flhist) / len(_flhist)) if _flhist else _total_lat / _n
            _avg_ttft = (sum(_thist) / len(_thist)) if _thist else None
            for _key, _val in (("avg_latency_s", _avg_lat), ("avg_tps", _avg_vel)):
    ...
    # FORK: active_task_count expands a delegate_task batch to its running children (N, not 1 pool slot).
    with contextlib.suppress(Exception):
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # FORK: currently-open tool call, so a reconnect/resume mid-tool-call renders its
            # pending row instead of a bare "thinking" bubble (see _open_tool_call_snapshot).
            "open_tool_calls": {},
...
def _open_tool_call_snapshot(session: dict) -> dict | None:
    ...
    if not isinstance(open_calls, dict) or not open_calls:
        return None
    # Only one tool call is realistically open at a time for the primary turn; if more than one is
    # somehow tracked, surface the most recent.
    _tool_call_id, call = max(open_calls.items(), key=lambda item: item[1].get("started_at") or 0)
...
def _deferred_session_record(
    ...
    return {
        ...
        # FORK: "open_tool_calls" tracks the currently-open tool call so a reconnect/resume
        # mid-tool-call renders its pending row (see _open_tool_call_snapshot).
        "tool_started_at": {}, "open_tool_calls": {}, "todo_state": todo_state,
        "transport": current_transport() or _stdio_transport,
        "auth_user_id": _transport_auth_user_id(current_transport()),
    }
...
def _notification_event_should_hold_for_liveness(evt: dict) -> bool:
    """True when a completion event must be held (requeued) because its owning subagent is still running.

    FORK-ONLY. Mirrors the drain_notifications liveness gate in tools/process_registry.py so the raw
    TUI poller and the drain agree on hold semantics: a completion event whose owning subagent is
    STILL running must not bubble into the top-level chat mid-task (the child's real identity lives
    in task_id "sa-N-..." while session_key carries the parent's key, so the ownership checks would
