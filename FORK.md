...
| `agent/system_prompt.py` | +52 / -23 | **2026-07-04**: both described fork features (date-only timestamp, grok in OPENAI_MODEL_EXECUTION_GUIDANCE) are now in upstream verbatim. Remaining fork diff is warm-memory status block + cache-breakpoint comments. |
