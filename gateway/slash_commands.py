...
class GatewaySlashCommandsMixin(
    ...
    async def _handle_stop_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        ``/stop <delegation_id>`` targets exactly ONE in-flight delegate_task dispatch (the id
        surfaced in the dispatch confirmation and in ``/agents``) without interrupting the
        foreground agent or any other background delegation — mirrors the CLI's targeted
        variant. A bare ``/stop`` keeps its whole-session-interrupt behavior below."""
