...
async def _handle_runs(self, request: "web.Request", *, _api_server) -> "web.Response":
    ...
    # Audit-log the submission (no-op if API_SERVER_AUDIT_LOG unset). Records the resolved
    # principal (stamped by _check_auth) and a SHA-256 of the prompt so the user text never
    # lands in audit logs; the run record itself still holds the content for legitimate
    # callers. Remote address comes from peername — under ``tailscale serve`` that is the
    # tailscale daemon on loopback, recorded anyway for completeness.
    try:
