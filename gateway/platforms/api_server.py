...
class APIServerAdapter(OpenAICompatRoutesMixin, BasePlatformAdapter):
    ...
    def _expected_api_key(self) -> str:
        """Return the API key authorized for the URL-selected profile (#72041)."""
    ...
    @staticmethod
    def _stamp_principal(request: "web.Request", principal: str) -> None:
        """Best-effort audit stamp; unit-test stubs are not always mappings."""
        try:
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        Side effect on success: stamps the resolved principal on ``request["principal"]`` —
        ``"default"`` on the legacy single-key path, the matching ``API_SERVER_KEYS_FILE`` entry
        on the multi-principal path, the profile name for a named profile, ``"anonymous"`` when
        no key material is configured at all.
        """
        profile = _api_request_profile.get()
        is_named_profile = bool(profile and profile != "default")
        expected_key = self._expected_api_key()
        if not expected_key:
            # Named profiles fail closed. The default listener keeps the historical allow-all
            # only when there is NO key material at all: a principals map with an unset
            # API_SERVER_KEY is configured auth, not an unconfigured server.
            if not is_named_profile and not self._principals:
                self._stamp_principal(request, "anonymous")
                return None
            if is_named_profile:
                logger.warning(
        ...
        if auth_header.startswith("Bearer "):
            ...
            if expected_key and hmac.compare_digest(token.encode(), expected_key.encode()):
                self._stamp_principal(request, profile if is_named_profile else "default")
                return None
            # Multi-principal path (API_SERVER_KEYS_FILE), default listener only — a named
            # profile is bound to its own scoped key and must never match the shared map.
            # Iterate every entry so wall-clock stays constant regardless of which (or no)
            # principal matches; accumulate instead of short-circuiting.
            if not is_named_profile:
                ...
                if matched is not None:
                    self._stamp_principal(request, matched)
                    return None
