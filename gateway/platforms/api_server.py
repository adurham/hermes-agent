...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        Profile-bound authentication (#72041): a request routed to a NAMED
        profile (``/p/<profile>/…``) is validated against that profile's own
        scoped ``API_SERVER_KEY`` — never the default listener's key or the
        principals map — and fails closed when the profile has no usable key.
        ...
        """
        _profile = _api_request_profile.get()
            ...
            if expected_key:
                ...
                if auth_header.startswith("Bearer "):
                    ...
                    if hmac.compare_digest(token.encode(), expected_key.encode()):
                        try:
                            request["principal"] = _profile
                        except TypeError:
                            # Non-mapping request stub (unit tests); the
                            # principal stamp is best-effort audit metadata.
                            pass
                        return None
            else:
                logger.warning(
                    "API server rejected request for profile %r: no "
                    "profile-scoped API_SERVER_KEY is configured; %s",
                    _profile,
