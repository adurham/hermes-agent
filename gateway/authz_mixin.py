...
class GatewayAuthorizationMixin:
    ...
    def _adapter_flag(self, platform, name: str, profile, *, source: Optional[SessionSource] = None) -> bool:
        ...
        When a *source* is supplied, its in-process transport adapter is consulted FIRST (issue #80884).
        Under ``gateway.multiplex_profiles`` one process-level adapter (A2A's stdlib http.server, Relay's
        connector WS) may serve inbound traffic routed to a SECONDARY profile while being registered only
        in the primary profile's map; a pure ``(platform, source.profile)`` lookup misses and default-denies
        a legitimately upstream-authorized event. These flags are intrinsic properties of the transport
        that actually received the request, not per-profile policy. Sources with no transport provenance
        (restored/hand-built) fall back to the profile-keyed lookup.
        """
        if not platform:
            return False
        if source is not None:
            # Upstream renamed the fork's ``_registered_transport_adapter`` helper to
            # ``_transport_owner`` (returns ``(adapter, profile)``); same registered-adapter check.
            owner = self._transport_owner(source)
            if owner is not None and bool(getattr(owner[0], name, False)):
                return True
    ...
    def _chat_scoped_grant(self, source, adapter_profile, is_group: bool, allow_adapter_delegation: bool) -> bool:
        ...
        if allow_adapter_delegation and (
            ...
            or self._adapter_flag(source.platform, "authorization_is_upstream", adapter_profile, source=source)
        ):
            return True
