...
def _bridge_auxiliary_config_to_env(_auxiliary_cfg: dict) -> None:
    ...
    for _task_key in _aux_bridged_keys:
        # Resolve through the schema-aware flattener so this works for BOTH the legacy task-first
        # config AND the provider-first schema (fork, 2026-06-24). For provider-first, reading
        # ``_auxiliary_cfg.get(_task_key)`` directly would only see the inert task-key pollution
        # injected by the DEFAULT_CONFIG merge (provider=auto, model=''); the flattener instead
        # selects the block matching the active main provider. Falls back to the raw dict if the
