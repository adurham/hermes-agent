...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo adurham/hermes-agent "<your terms>"
  gh search prs --repo adurham/hermes-agent --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/adurham/hermes-agent/issues?q=) · [PRs (all states)](https://github.com/adurham/hermes-agent/pulls?q=is%3Apr).
...
- Implement the relevant ABC and use the existing plugin discovery path (`~/.hermes/plugins/`, project `.hermes/plugins/`, or a pip entry point) — see [Build a Hermes Plugin](https://github.com/adurham/hermes-agent/blob/main/website/docs/developer-guide/plugins/index.md)
...
- Use [GitHub Issues](https://github.com/adurham/hermes-agent/issues)
