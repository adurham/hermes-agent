...
_HERMES_CORE_TOOLS = [
    ...
    "skills_list", "skill_view", "skill_manage", "skill_pitfalls",
...
TOOLSETS = {
    ...
    # FORK: local agent messaging (docs/design/local-agent-messaging.md). Opt-in.
    # send_agent_message is parent/session-only and is additionally withheld from
    # gateway-origin sessions via its check_fn; send_to_parent is granted to a
    # delegated child only under background=true (a synchronous child's parent
    # thread is blocked and can't act on anything sent to it). list_agents lives in
    # the separate "agent_visibility" toolset below, granted to every delegated
    # child (synchronous or background) once its parent has cross_session enabled:
    # read-only visibility has no "parent can't react" gating rationale the way the
    # SEND tools do. Names are pinned by tools/agent_messaging_contract.py
    # (TOOLSET_NAME / TOOLSET_NAME_VISIBILITY).
    "cross_session": _ts(
        "Message other local Hermes participants: other live sessions or your own "
        "running subagents (send_agent_message), or the parent that delegated to "
        "you (send_to_parent)",
        ["send_agent_message", "send_to_parent"],
        # Enabling "cross_session" (the one config.yaml/`hermes tools` toggle users
        # actually see) also brings in list_agents via this include, so a top-level
        # session's UX is unchanged from before the split. Only the delegated-child
        # gating differs (see tools/delegate_tool.py).
        includes=["agent_visibility"],
    ),
    # FORK: read-only machine-wide agent/subagent awareness (list_agents). Split out
    # of "cross_session" so it can be granted to a delegated child regardless of
    # background=true/false. Still opt-in: only reaches a child when the parent has
    # "cross_session" enabled, so it stays one user-facing switch.
    "agent_visibility": _ts(
        "Read-only: list every other live Hermes session and subagent on this "
        "machine (list_agents) -- goal, cwd, owner, status. Use this to notice a "
        "working-directory collision with concurrent work before editing files.",
        ["list_agents"],
    ),
    # FORK: second-opinion tool backed by auxiliary.consult in config.yaml.
    "consult": _ts(
        "Get a second opinion from a configurable reference model "
        "(auxiliary.consult in config.yaml) before a risky or uncertain decision. "
