...
class HermesACPAgent(SlashCommandsMixin, acp.Agent):
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            # FORK: this rebuild happens mid-conversation (ACP MCP server registration on an
            # already-running session), so it must honor and update the sticky tool_search
            # activation latch the same way refresh_agent_mcp_tools does — otherwise a shrinking
            # live registry here could flip bridge tools off and corrupt tool-call history via
            # _strip_unknown_tool_blocks. See tools/tool_search.py::assemble_tool_defs docstring.
            ...
            # FORK: keep the sticky tool_search activation latch in sync (see the comment on the
            # get_tool_definitions call above) — a rebuild that shows the bridge must latch it.
            try:
