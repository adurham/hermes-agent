...
| `/stop [delegation_id]` | Bare `/stop` kills all running background processes. `/stop <delegation_id>` targets exactly ONE in-flight `delegate_task` dispatch (single task or whole batch) by the id shown in its dispatch confirmation and status-bar line, cancelling it without touching any other running background work — use this to pull back a mis-dispatched subagent (wrong model, stale instructions, duplicate work) instead of killing everything. |
...
| `/stop [delegation_id]` | Bare `/stop` kills all running background processes and interrupts the running agent. `/stop <delegation_id>` cancels exactly one in-flight `delegate_task` dispatch (single task or whole batch) without touching anything else running — mirrors the CLI's targeted cancel. |
