...
# A single length-truncated turn with in-progress reasoning-channel content and no
# visible answer yet is not necessarily exhaustion — the very next call can legitimately
# produce a real answer (upstream's own invariant, tests/agent/test_empty_terminal_reasoning_surface.py
# ::test_length_cut_reasoning_is_not_promoted: content=None, real reasoning, finish_reason='length'
# on call 1 must still continue to call 2). What IS a genuine, worth-aborting-on problem is the
# SAME symptom repeating within one turn — a model stuck re-exhausting its reasoning budget on every
# retry with zero progress (the fork's original incident: 6x in 15 min, never recovering). Gate the
# reasoning-channel branch on `length_continue_retries` so it fires on the SECOND-and-later
# reasoning-exhausted length-cut, not the first.
_MAX_REASONING_ONLY_LENGTH_CUTS_BEFORE_ABORT = 1
...
def _abort_reason(
    ...
    FORK: ``reasoning`` covers models (DeepSeek-V4, Moonshot) that stream chain-of-thought in
    the SEPARATE ``reasoning_content``/``reasoning`` field instead of inline <think> tags in
    ``content``. When such a model burns its whole output budget there it returns
    finish_reason='length' with substantial reasoning but empty/no content and no tool calls —
    the inline-tag check above never sees it, so the turn would otherwise fall through to
    pointless continuation retries (each re-sending the same prompt and exhausting again).

    ``length_continue_retries`` (this turn's continuation-attempt count so far) gates that
    branch: fire on the SECOND-and-later reasoning-exhausted length-cut, not the first — one
    genuine continuation attempt is cheap and can succeed (see
    _MAX_REASONING_ONLY_LENGTH_CUTS_BEFORE_ABORT's docstring reference above), while a repeat
    of the identical symptom is the real "stuck, never recovering" case this branch exists for.
    """
    if has_tool_calls:
        return None
    ...
    content_is_blank = not (isinstance(content, str) and content.strip())
    if (
        content_is_blank and reasoning and str(reasoning).strip()
        and length_continue_retries >= _MAX_REASONING_ONLY_LENGTH_CUTS_BEFORE_ABORT
    ):
        return _THINKING_EXHAUSTED
...
def _continue_text(st: _Trunc, _retry: TurnRetryState, assistant_message: Any) -> TruncationVerdict:
    ...
    # When the stream died before ANY visible output was salvaged (content empty, no
    # tool calls, no dropped-tool stub — e.g. a backend that clean-fails a malformed
    # tool call mid-stream after only reasoning deltas), appending the EMPTY assistant
    # turn plus "continue exactly where you left off / do not restart or repeat prior
    # text" is actively harmful: there is nothing to continue FROM, and the instruction
    # forbids restarting. Observed live (2026-07-27, exo DSv4-Flash on hard_eval coding
    # tasks): the model obediently "continues" from nothing — emitting an empty
    # <think></think> turn — and each retry adds another empty assistant turn to
    # history, teaching it that empty turns are acceptable until every retry budget
    # exhausts and the turn ends empty_response_exhausted. Retry the SAME request with
    # NO history mutation instead: sampling diversity gets a fair re-roll, exactly like
    # an HTTP-level retry.
    if (
        st.is_stub
        and not (_interim_content or "").strip()
        and not _dropped_tools
        and n < 4
    ):
        agent._vprint(
            f"{agent.log_prefix}↻ Stream failed before any visible output — "
            f"retrying cleanly ({n}/4)..."
        )
...
def recover_from_truncation(
    ...
    # FORK: reasoning-channel exhaustion (DeepSeek-V4, Moonshot stream CoT in this
    # separate field rather than inline <think> tags) — see _abort_reason docstring.
    _trunc_reasoning = (
        getattr(_trunc_msg, "reasoning_content", None) or getattr(_trunc_msg, "reasoning", None)
    ) if _trunc_msg else None

    abort = _abort_reason(
        agent, _trunc_content, _trunc_has_tool_calls, _trunc_reasoning,
