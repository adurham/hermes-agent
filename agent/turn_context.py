...
def build_turn_context(
    ...
    # FORK (Phase 3 auto-feedback): bind session_id to a contextvar so ``WarmStore.recall``
    # can stash recall results in the per-session window without threading session_id
    # through the memory tool surface. See agent/fork/memory_recall.py.
    try:
    ...
    # FORK: capture the user message text for the memory-recall reminder
    # (agent/fork/memory_recall.py), which uses the user's framing as the primary seed
    # for query-candidate extraction. Best-effort — non-string (multimodal) inputs get
    # the str() form. Each new user turn also resets the recent-tool-args window: args
    # from a prior turn must not seed a query for the new investigation.
    try:
        agent._last_user_message = user_message if isinstance(user_message, str) else str(user_message or "")
    except Exception:
    ...
    # FORK: delegation identity (persona / role), stashed on the child by
    # tools/delegate_tool._build_child_agent (``_delegate_agent_type`` / ``_delegate_role``).
    # Only subagents carry these — the top-level session has neither, so the getattr
    # defaults keep this safe for CLI/gateway/cron/test agents. This is the only per-turn
    # log site where "which role was this session dispatched as" is answerable from
    # agent.log alone; model= alone is ambiguous when several roles share one model.
