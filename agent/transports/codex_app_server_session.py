...
# Hermes choices: once/session/always/deny/timeout. Codex wire values:
# accept/acceptForSession/decline/cancel (codex-rs/app-server-protocol/src/protocol/v2/item.rs,
# codex 0.130.0). FORK: an unanswered prompt maps to "cancel" (request withdrawn), not "decline"
# (explicit user refusal); codex does not run the command either way. Anything unmapped -> decline.
_APPROVAL_CHOICE_TO_DECISION = {
    "once": "accept", "session": "acceptForSession", "always": "acceptForSession", "timeout": "cancel",
}
