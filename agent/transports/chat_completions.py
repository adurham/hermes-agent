...
def _apply_max_tokens(api_kwargs: dict, model: str, reasoning_config: Any, params: dict, profile_max: Any = None) -> None:
    ...
    FORK: ``anthropic_max_output`` is a last-resort fallback (below) — applied only when
    no ephemeral/user/profile max_tokens was set. Model-gated, not URL-gated: any
    chat-completions proxy serving a Claude/MiniMax/Qwen3 model needs max_tokens, because
    the Anthropic Messages API treats it as mandatory and proxies that omit it (AWS
    Bedrock, NVIDIA, LiteLLM, vLLM, corporate gateways) default as low as 4096 output
    tokens — easily exhausted by thinking + large tool calls like write_file/patch. Set as
    plain ``max_tokens`` (not through ``max_tokens_fn``) since the aggregator proxies to
    the Anthropic Messages API, which requires the field under that exact name.
    """
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(self, messages: list[dict[str, Any]], **kwargs) -> list[dict[str, Any]]:
        ...
        FORK: for the exo provider (``provider`` kwarg, exact ``exo`` / ``custom:exo``) messages are
        additionally rebuilt in a frozen byte-stable key order and any whitespace-only
        ``reasoning_content`` pad is omitted — see ``agent.exo_canonical_serializer`` for the
        prefix-cache contract. Every other provider's output is byte-for-byte unchanged; pass
        ``provider=None`` (the default) to opt out entirely.
        """
        # FORK: exo canonical serializer runs first (no-op for every non-exo provider).
    ...
    def build_kwargs(
        ...
        """
        # FORK: pass provider so the exo canonical serializer (byte-stable key order + pad
        # omission) is applied at this last-touch chokepoint — no-op for every non-exo provider.
        sanitized = self.convert_messages(messages, model=model, provider=params.get("provider"))
        ...
        if supports_reasoning and not is_lmstudio:
            if params.get("is_github_models", False):
                ...
                # FORK: the Nous portal rejects an explicit disabled-reasoning block; omit it there.
                if not (is_nous and off):
                    extra_body["reasoning"] = {"enabled": not off, "effort": "none" if off else _effort}

        # FORK: Nous portal usage tags.
        if is_nous:
        ...
        # FORK: custom OpenAI-compatible providers:
        #   • Ollama-style: extra_body["think"] = False to disable.
        #   • OpenAI-Responses-style (exo, vLLM with reasoning, LM Studio when not auto-detected):
        #     top-level reasoning_effort is the standard Chat Completions field for o-series and is
        #     accepted by the Python SDK. ``enable_thinking`` is an exo extension and must go in
        #     extra_body or the SDK raises TypeError("unexpected keyword argument").
        if params.get("is_custom_provider", False):
            if reasoning_config and isinstance(reasoning_config, dict):
                _c_effort = (reasoning_config.get("effort") or "").strip().lower()
                _c_enabled = reasoning_config.get("enabled", True)
                if _c_effort == "none" or _c_enabled is False:
                    extra_body["think"] = False
                    extra_body["enable_thinking"] = False
                elif _c_effort:
                    api_kwargs["reasoning_effort"] = _c_effort
                    extra_body["enable_thinking"] = True

        # FORK: Qwen portal (Alibaba DashScope OpenAI-compat endpoint) high-res image flag. The
        # param is is_qwen_portal — an earlier refactor referenced an undefined ``is_qwen`` here and
        # raised NameError on every legacy-path call ("name 'is_qwen' is not defined" in errors.log).
