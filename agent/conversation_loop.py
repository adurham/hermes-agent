        ...
        try:
            ...
            if session_row is not None:
                ...
                if raw_prompt is None:
                    ...
                    # FORK back-compat (the ONLY surviving sentinel strip): sessions persisted by
                    # the retired in-band cache-split mechanism (FORK.md 2026-06-02, removed
                    # 2026-08-04) stored the internal SYSTEM_VOLATILE_SENTINEL inside the system
                    # prompt. Strip it here — the single point where a historical prompt enters
                    # ``_cached_system_prompt`` — so the marker can never leak to the model.
                    # Stripping restores the exact "\n\n" join those sessions actually sent on the
                    # wire, so their prefix cache still matches. No-op for prompts written after
                    # the removal.
                    from agent.prompt_caching import strip_volatile_sentinel

                    stored_prompt = strip_volatile_sentinel(raw_prompt) or None
