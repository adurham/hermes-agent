...
def run_conversation(
    ...
    # Note: the fork's inline "cancel an in-flight background review at the
    # start of a live turn" block (#67821) is gone from here on purpose —
    # upstream converged on the same behavior as
    # agent.background_review.cancel_background_review_for_live_turn(), which
    # run_agent.py already calls on the live-turn path and which additionally
    # awaits the review's request-phase acknowledgement.
    ...
    # ``agent/turn_context.py``.  Fork note: the fork-only prologue steps
    # (memory_auto_feedback session bind, _last_user_message capture,
    # _recent_tool_args reset) were ported INTO build_turn_context during the
    # 2026-06-08 sync — keep them there, not re-inlined here.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # Auth/retry single-shot flags consolidated into TurnRetryState (_retry)
        # upstream — referenced as _retry.<flag> below.  Two fork-only flags
        # have no TurnRetryState home and stay bare locals:
        #   _strip_cache_for_overload    — cache-strip-on-overload recovery
        #   _refusal_sanitize_attempted  — refusal-sanitize ladder (fork-only)
        # NOTE: max_compression_attempts is NOT re-declared here — upstream
        # moved it to a configurable `getattr(agent, "max_compression_attempts", 3)`
        # earlier in this function (see above); re-hardcoding it to 3 here
        # would silently clobber that configured value.
                    ...
                    if agent.api_mode in {"chat_completions", "bedrock_converse", "anthropic_messages"}:
                        ...
                        if assistant_message is not None and not _trunc_has_tool_calls:
                            ...
                            _is_empty_partial_stub = (
                                _is_partial_stream_stub and not _salvaged_text
                            )
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                _compressor = agent.context_compressor
                if _compressor.last_prompt_tokens > 0 and not getattr(
                    _compressor, "last_server_tool_requests", 0
                ):
                ...
                if (
                    ...
                    def _int_attr(obj, name):
                        # Coerce defensively: test doubles / plugin engines may
                        # expose non-int attrs (e.g. MagicMock auto-attrs).
                        try:
                            return int(getattr(obj, name, 0) or 0)
                        except (TypeError, ValueError):
                            return 0

                    _cr = _int_attr(_compressor, "last_cache_read_tokens")
                    _in = _int_attr(_compressor, "last_input_tokens")
                    _cw = _int_attr(_compressor, "last_cache_write_tokens")
