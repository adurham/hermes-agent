...
non-vision models). :func:`decide_image_input_mode` picks once per turn from
``agent.image_input_mode`` (``auto`` | ``native`` | ``text``). In ``auto`` native
capability is checked FIRST: a model reporting ``supports_vision`` attaches the
pixels directly, beating a lossy summary even when ``auxiliary.vision`` is set.
Only when the main model cannot see does an explicit ``auxiliary.vision`` backend
take over, and then ONLY for the local exo cluster (FORK: vision delegation is
exo-scoped, so a non-exo, non-vision model native-attaches rather than pulling exo
into an unrelated provider's request); otherwise the historical text pipeline runs.
...
def _file_to_data_url(path: Path) -> Optional[str]:
    """Encode a local image as a base64 data URL, downscaling if the encoding would
    exceed ``_NATIVE_IMAGE_CEILING_BYTES`` (images already under it keep native size,
    so screenshots and normal uploads pay no quality tax); MIMEs outside the accepted
    set are transcoded to PNG. None when unreadable, blocked by the read guard, or
    untranscodable — the caller reports ``skipped``."""
    try:
    ...
    # Transcode formats the active provider does not accept (the managed local
    # runtime accepts fewer than the universal set) to PNG.
