...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            # when the backing OAuth file is gone.  keychain_longlived is
            # handled the same way: is_borrowed_credential_source() returns
            # True for it (not in _PERSISTABLE_PROVIDER_SOURCES), so it is
            # kept only while active_sources re-seeds it from the keychain.
