...
def _is_local_validation_error(api_error: Any) -> bool:
    ...
    response.completed.output=null — retryable so the fallback path runs), and the
    Anthropic SDK's "Unable to parse tool parameter JSON" ValueError (streamed
    input_json_delta corruption off the wire, not a local bug — the SDK itself asks the
    caller to retry; #39021)."""
    ...
    if "unable to parse tool parameter json" in _text:
        return False
