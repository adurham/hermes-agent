...
def compress_context(
    ...
    # Fork Phase 2 auto-extraction: piggyback on the compression boundary to
    # extract durable facts from the slice that's about to be discarded. No-op
    # when memory.auto_extract is off in config. Best-effort; never blocks
    # compression. Kept inline in compress_context (not in _run_summary_phase)
    # so it runs exactly once per boundary, before any message is summarised.
    try:
