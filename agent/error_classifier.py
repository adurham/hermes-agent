...
# Python builtins that can ONLY come from a bug in OUR code path, never a
# transient provider/transport failure: retrying re-runs the identical broken
# code (real case: a missing `import logging` in a per-turn diagnostics helper
# raised NameError on every attempt and killed the session with 0 tool calls,
# 2026-07). Matched by EXACT builtin type name so provider SDK classes are not
# swept in. Deliberately CONSERVATIVE — AttributeError / TypeError / KeyError /
# IndexError / ValueError are excluded: those can legitimately arise from a
# malformed provider response where a retry yields a valid one.
...
# A bug in our own code inside the API call path is deterministic: fail fast
# instead of burning the retry budget on a flaky-provider impersonation.
_V_INTERNAL_CODE_ERROR = _v(_R.internal_code_error, retryable=False)
...
_ERROR_CODE_VERDICTS: Dict[str, Verdict] = {
    **dict.fromkeys(("overloaded_error", "overloaded"), _V_OVERLOADED),
...
def _provider_special_cases(c: _Ctx) -> Optional[Verdict]:
    ...
    # Two wordings: legacy "Invalid signature in thinking block", and the
    # clear_thinking_20251015 strict-validation "thinking or redacted_thinking
    # blocks in the latest assistant message cannot be modified". Same recovery.
...
def _by_transport(c: _Ctx) -> Optional[Verdict]:
    ...
    if transport:
        return _V_TIMEOUT
    # Internal code bug (NameError/ImportError/…) → fail fast. Exact builtin
    # name AND isinstance, so a provider SDK class reusing one of these names
    # can't slip in while subclasses (UnboundLocalError, ModuleNotFoundError,
    # IndentationError) still match. Runs AFTER the transport check above so
    # genuine OSError/ConnectionError/TimeoutError keep their retryable
    # timeout classification.
    if c.error_type in _INTERNAL_CODE_ERROR_TYPES or isinstance(
        c.error, (NameError, ImportError, NotImplementedError, SyntaxError)
    ):
        return _V_INTERNAL_CODE_ERROR
    return None
