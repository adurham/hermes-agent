...
class InsightsEngine:
    ...
    def generate(self, days: int = 30, source: str = None,
                 account_billed: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Generate a complete insights report for the last ``days`` days, optionally filtered by source platform.

        ``account_billed`` is optional authoritative billed-spend info from the provider's
        usage API (e.g. Claude's ``extra_usage.used_credits``), shaped
        ``{"billed_usd": float, "currency": str, "monthly_limit_usd": float|None}``. It is
        period-cumulative (NOT windowed by ``days``), so the cost section labels it
        distinctly from the windowed token-based estimate."""
    ...
    def _compute_overview(self, sessions: List[Dict], message_stats: Dict, models: Optional[List[Dict]] = None) -> Dict:
        ...
        return {
            ...
            "estimated_cost_sessions": status_counts["estimated"],
            "actual_cost_sessions": status_counts["actual"],
            # Any dollar figure at all (actual or estimated) — #77223 follow-up:
            # bucketing only included/unknown left the common "estimated" case
            # uncounted and made priced_cost_sessions read 0.
            "priced_cost_sessions": status_counts["actual"] + status_counts["estimated"],
        }
    ...
    def _format_cost_lines(overview: Dict, account_billed: Optional[Dict]) -> List[str]:
        ...
        if priced_sessions > 0 and estimated > 0:
            ...
            # No provider-billed figure and no fully-priced sessions to anchor
            # an "Est. (this window)" line against, but there IS a nonzero
            # estimate — surface it plainly so a sub-cent aggregate doesn't
            # silently vanish (#79220 formatting fix reused via _fmt_est_cost).
            lines.append(f"  Estimated:           {_fmt_est_cost(estimated)}")

        # Included/Unknown bucket lines (#77223): subscription-included and
        # unpriced sessions must be visible instead of silently collapsing
        # to $0 or disappearing from the report entirely.
        included_sessions = overview.get("included_cost_sessions") or 0
        if included_sessions > 0:
            lines.append(
                f"  Included:            {included_sessions} session(s) "
                f"(subscription — no provider invoice)"
            )
        if unknown_sessions > 0 and not (priced_sessions > 0 and estimated > 0):
            # Only add a standalone Unknown line when it wasn't already
            # folded into the "excluded" note on the Est. line above.
            lines.append(
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        # Cost. The authoritative billed figure (provider usage API) anchors the
        # section when available; it is period-cumulative, not windowed by --days,
        # so it is labeled distinctly from the token list-price estimate. Bucket
        # lines keep included/unknown sessions visible instead of collapsing to $0.
