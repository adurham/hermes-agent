...
def apply_reasoning_content_policy(source_msg: dict, api_msg: dict, needs_thinking_pad: bool,
                                   omit_pad: bool = False) -> None:
    """Copy provider-facing reasoning fields onto an API replay message (mutates ``api_msg``).
    ``needs_thinking_pad`` is the require-side flag (``needs_reasoning_echo``). ``omit_pad``
    (exo-specific) suppresses the single-space pad while preserving genuine reasoning verbatim."""
    ...
    if isinstance(existing, str):
        ...
        # exo (``omit_pad``): a blank/whitespace value IS the synthetic pad — omit the key entirely
        # so one space cannot defeat the server's prefix-cache LCP on the next turn's replay.
        # Genuine (non-whitespace) reasoning is still echoed verbatim, even on exo.
        if omit_pad and not existing.strip():
            api_msg.pop("reasoning_content", None)
        else:
        ...
        # exo: never inject the synthetic pad (LCP defeat). Foreign CoT is still not leaked —
        # the key is simply absent, which exo treats identically to an empty value.
        api_msg.pop("reasoning_content", None)
    else:
...
def reapply_reasoning_echo(api_messages: list, needs_thinking_pad: bool, omit_pad: bool = False) -> int:
    ...
    ``omit_pad`` (exo-specific) suppresses the single-space pad while keeping the
    require-side reconciliation intact.
    """
    changed = 0
