...
def init_agent(
    ...
    # ``_background_review_agent`` holds the forked review AIAgent while its
    # run_conversation() is in flight, so the NEXT live turn can proactively
    # interrupt a still-running review instead of letting the two race
    # concurrently against the same session_id/credentials (#67821 — observed
    # as doubled prompt-token counts and a Ctrl+C-proof lockup when a live turn
    # started before a review fired at the end of the prior turn had finished).
