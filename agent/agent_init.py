...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    # FORK: pre-stamp the 1M-context-beta latch for models with no 1M tier. Haiku 4.5 (and
    # pre-4.6 Claude) reject ``context-1m-2025-08-07`` even on paid keys — the entitlement is
    # per-model. Stamping up front means the header never goes out, so no 400, no noisy retry.
    # ``_oauth_1m_beta_disabled`` is what every downstream client-build path already keys on.
    try:
        ...
        pass  # best-effort — never let the gate crash agent init
...
_CONTROL_STATE: Dict[str, Any] = {
    ...
    # FORK: set by every accepted redirect() — True when it degraded to a queued steer()
    # (mid-tool-call) rather than a live-cancel redirect, so surfaces can print an honest
    # confirmation instead of always claiming "Redirected current turn".
    "_last_redirect_degraded_to_steer": None,
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    # Upstream's route classifier subsumes the fork's inline check: it owns the native-host /
    # ``anthropic``-provider gate before calling ``_is_oauth_token`` on a materialized credential.
    agent._is_anthropic_oauth = anthropic_route_is_oauth(base_url, effective_key, provider=agent.provider)
    # FORK: honor the 1M-beta latch pre-stamped in _finalize_routing at FRESH construction, not
    # just on rebuild — otherwise every Haiku-routed agent builds a client carrying
    # ``context-1m-2025-08-07``, 400s on the first call and only gets clean on rebuild.
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    ...
    # FORK: sticky tool_search activation flag, read BEFORE the first get_tool_definitions()
    # call so it defaults to False here (init is this agent's first assembly). Once activated it
    # stays activated for the life of the agent, so a live-registry token shift can never flip
    # bridge tools out of the wire array mid-conversation and corrupt tool-call history via
    # _strip_unknown_tool_blocks. See tools/tool_search.py::assemble_tool_defs.
    ...
    # FORK: latch the sticky flag once the bridge tools actually shipped.
    try:
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # FORK: fork-only feature state — each module owns its own init.
...
def _apply_agent_section(agent, _agent_cfg):
    ...
    # FORK: skill-recall reminder interval (0 disables).
    with suppress(Exception):
        agent._skill_recall_reminder_interval = int(
            _cfg_dict(_agent_cfg, "skills").get("recall_reminder_interval", 6)
        )

    # FORK: memory-recall reminder + session-pin caps (see agent/fork/memory_recall.py and
    # agent/fork/memory_session_pin.py). Defaults already set by their init_state(); bad config
    # keeps them. Keys: agent.memory.{recall_reminder_interval, recall_reminder_mode,
    # recall_auto_top_k, recall_min_user_chars, session_pin_max_count, session_pin_max_chars}.
    try:
        _memory_cfg = _cfg_dict(_agent_cfg, "memory")
        agent._memory_recall_reminder_interval = int(_memory_cfg.get("recall_reminder_interval", 8))
        _mode = str(_memory_cfg.get("recall_reminder_mode", "auto")).lower().strip()
        agent._memory_recall_reminder_mode = _mode if _mode in ("auto", "hint") else "auto"
        agent._memory_recall_auto_top_k = max(1, int(_memory_cfg.get("recall_auto_top_k", 3)))
        agent._memory_recall_min_user_chars = max(0, int(_memory_cfg.get("recall_min_user_chars", 200)))
        agent._session_pin_max_count = max(1, int(_memory_cfg.get("session_pin_max_count", 5)))
        agent._session_pin_max_chars = max(100, int(_memory_cfg.get("session_pin_max_chars", 2000)))
    except Exception:
        pass

    # FORK: consult-nudge interval (consult.nudge_interval; 0 disables the nudge only — the
    # ``consult`` tool itself stays available).
    with suppress(Exception):
        agent._consult_nudge_interval = max(0, int(_cfg_dict(_agent_cfg, "consult").get("nudge_interval", 8)))
...
_USAGE_STATE: Dict[str, Any] = {
    ...
    # FORK: _api_latency_history holds DECODE-ONLY time so velocity is true decode throughput;
    # _api_full_latency_history keeps the unmodified call wall-clock that the latency readout
    # uses, and _api_ttft_history records TTFT per streamed call (appended only when a first
    # delta actually fired).
    "_api_ttft_history": lambda: deque(maxlen=10),
    "_api_full_latency_history": lambda: deque(maxlen=10),
}
...
def init_agent(
    ...
    # tool. For models that emit one <think> per turn but chain reasoning across turns.
    ...
    agent.interleaved_thinking = bool(interleaved_thinking)  # FORK
    agent.request_overrides = dict(request_overrides or {})
    agent.prefill_messages = prefill_messages or []  # Prefilled conversation turns
    agent._force_ascii_payload = False
    if agent.max_tokens is None:
        # FORK: per-model config default (custom_providers[*].models.<id>.max_tokens) so thinking
        # models keep enough generation budget for reasoning_content + the visible response.
        try:
