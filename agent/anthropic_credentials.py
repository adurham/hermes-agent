...
def _read_claude_code_credentials_from_keychain() -> Optional[Dict[str, Any]]:
    """Read a Claude Code OAuth Keychain entry (Claude Code >=2.1.114).

    FORK: enumerates every service name starting with "Claude Code-credentials"
    (see ``_list_claude_code_keychain_service_candidates``), newest-modified first, and
    returns the first with a non-empty, well-formed payload — some installs suffix the
    service name per-install and leave a stale, empty unsuffixed entry behind.
    """
    if platform.system() != "Darwin":
        return None

    service_names = _list_claude_code_keychain_service_candidates()
    # Always try the plain exact name too (in case dump-keychain enumeration fails, e.g.
    # sandboxing) — de-duplicate, preserve order.
    ...
    for service_name in service_names:
        ...
        try:
            record = _claude_oauth_record(json.loads(raw), "macos_keychain")
        except json.JSONDecodeError:
        ...
        if record is not None:
            if service_name != "Claude Code-credentials":
                logger.debug("Keychain: using suffixed Claude Code credential entry '%s'", service_name)
            return record
        # Empty/malformed payload (e.g. a stale placeholder entry) — fall through
        # and try the next candidate rather than giving up.
...
def resolve_anthropic_token(*, model: Optional[str] = None) -> Optional[str]:
    ...
    FORK: by default a refreshable Claude Code credential preempts a static token from
    ANTHROPIC_TOKEN/CLAUDE_CODE_OAUTH_TOKEN so a stale persisted token never blocks
    auto-refresh. Set ``agent.pin_anthropic_token: true`` in config.yaml to invert that
    (see ``_pin_static_anthropic_token``).
    ...
    if token:
        if pin_static:
            return _available_anthropic_token(token, model)
