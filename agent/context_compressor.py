...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def record_completed_compaction(self, *, used_fallback: bool = False, feasibility_skip: bool = False) -> None:
        ...
        Fork: every call here — regardless of ``used_fallback``/``feasibility_skip`` — is a real completed
        compaction boundary, so it also increments the durable ``compression_attempts_total`` session counter.
        Unlike the anti-thrash counters (FAILURE signals that read 0 both when compression never fired and
        when it fired perfectly every time), this is a monotonic POSITIVE signal answering "did compression
        ever run for this session" — see FORK.md for the false-alarm this column exists to prevent.
        """
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _build_summary_prompt(
        ...
        _summarizer_preamble = (
            ...
            "For terminal commands that involve database operations (pg_dump, mysqldump, etc.), credential "
            "extraction (PGPASSWORD, SQLConnectionString, etc.), or data-transfer operations (aws s3, "
            "upload_stream, presigned URLs), describe the action in plain language rather than quoting the "
            "raw shell command. Preserve the key facts (database host, file path, outcome, tool used) but "
            "do not reproduce inline credential substitutions or flag-heavy command strings verbatim — a "
            "future reader needs to know WHAT was done and WHY, not the exact invocation."
        )
