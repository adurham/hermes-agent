...
SUMMARY_PREFIX = (
    ...
    "If the latest user message contradicts, supersedes, changes topic "
    "from, or in any way diverges from '## Active Task' / '## In Progress' "
    "/ '## Pending User Asks' / '## Remaining Work', the latest message "
    "WINS — discard those stale items entirely and do not 'wrap up the old "
    "task first'. Reverse signals in the latest message (e.g. 'stop', "
    "'undo', 'roll back', 'just verify', 'don't do that anymore', 'never "
    "mind', a new topic) must immediately end any in-flight work described "
    "in the summary; do not re-surface it in later turns. "
