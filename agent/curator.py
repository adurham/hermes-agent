...
def load_state() -> Dict[str, Any]:
    base: Dict[str, Any] = {
        ...
        # Fork: the most recent LLM review pass's refused skill_manage writes
        # (unmanaged/pinned/external/bundled/hub-installed skills) — see
        # _extract_blocked_writes / _background_review_write_guard. Replaced wholesale on
        # every LLM pass, not accumulated. The key filter below silently drops any persisted
        # key absent from this dict, so it MUST be listed here to round-trip a
        # save_state()/load_state() pair (that is what `hermes curator status` reads).
        "blocked_writes": [],
    }
    path = _state_file()
    try:
...
def get_prune_builtins() -> bool:
    ...
    FORK: also the switch the hot-tier audit reads to decide whether ``dead``/``stale``
    memory entries may be hard-deleted (``agent.hot_tier_audit._run_live_llm``). Restored
    after the v2026.9.21 merge silently dropped it when upstream replaced the
    PRUNE-BUILTINS-MODE prompt note with an unconditional bundled-skill exclusion.
    """
...
def _write_run_report(
    *, started_at: datetime, elapsed_seconds: float, auto_counts: Dict[str, int], auto_summary: str,
    ...
    payload = {
        ...
        # Fork: refused skill_manage writes from the review pass (see _extract_blocked_writes).
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    if final:
        ...
        lines += ["## LLM summary\n", p.get("llm_summary"), ""]
    # Fork: blocked writes — skill_manage calls the review fork's own write guard refused
    # (unmanaged/pinned/external/bundled/hub-installed skills). Deliberately its own section,
    # not buried in tool-call counts: each entry needs a manual command to actually take
    # effect, and the whole point is that a human reads it and acts on it.
    blocked = p.get("blocked_writes") or []
    if blocked:
        _SHOW = 25
        lines += [
            f"## Blocked writes — needs manual action ({len(blocked)})\n",
            "_The review fork tried to patch these skills but its own safety guard refused — "
            "these are skills the curator is NOT currently allowed to touch autonomously. "
            "Nothing is broken; each just needs one manual command to opt in (or is "
            "intentionally left alone)._\n",
        ]
        for entry in blocked[:_SHOW]:
            hint, reason = entry.get("hint", ""), entry.get("reason", "")
            lines.append(f"- `{entry.get('skill', '?')}` ({entry.get('action', '?')})" + (f" — `{hint}`" if hint else ""))
            if reason and not hint:
                lines.append(f"    - {reason}")
        lines += ([f"- … and {len(blocked) - _SHOW} more (see `run.json`)"] if len(blocked) > _SHOW else []) + [""]
...
def run_curator_review(
    ...
    *dry_run* SKIPS the stale/archive transitions and instructs the fork to report only; REPORT.md is still written and
    ...
    def _llm_pass():
        ...
        # Fork: surface skill_manage writes the review fork's own guard refused
        # (unmanaged/pinned/external/bundled/hub-installed skills — see
        # ``_background_review_write_guard``). These used to be visible ONLY as a per-tool-call
        # line in a live CLI's scrollback or by grepping agent.log — nothing rolled them up
        # anywhere a user would look after the fact. Append a short actionable summary line and
        # persist the full list to state so `hermes curator status` can show it on every
        # subsequent invocation, not just the one where it scrolled past in a terminal.
        ...
        if blocked_writes:
            ...
            final_summary = (
                f"{final_summary}\nblocked (needs manual action): {', '.join(_names)}"
                " — see `hermes curator status`"
            )
        elapsed = (datetime.now(timezone.utc) - start).total_seconds()
        state2 = {**load_state(), "last_run_duration_seconds": elapsed, "last_run_summary": final_summary}
        # Fork: persisted across runs until superseded — this run's set of writes the curator
        # wanted to make but couldn't without a manual `hermes curator adopt`/`unpin`. Replaced
        # wholesale each LLM pass (not accumulated); an empty list means the most recent pass hit
        # no refusals, which is itself useful signal for status to show.
    ...
    if synchronous:
        ...
        # Fork: track the thread so is_curator_running() can tell the CLI exit path whether a
        # pass is still in flight — its cost has not landed in the ledger yet if so.
...
def maybe_run_curator(*, idle_for_seconds: Optional[float] = None, on_summary: Optional[Callable[[str], None]] = None) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Upstream adds the interval gate here; fork appends the hot-tier audit pass below.
        if not should_run_now() or (idle_for_seconds is not None and idle_for_seconds < get_min_idle_hours() * 3600.0):
            return None
        # Upstream's single-flight claim: only one curator pass at a time across processes,
        # released in the finally below. Fork's hot-tier audit runs inside the claim so it
        # cannot overlap another process's skill pass.
        if not _claim_run():
            return None
        try:
            result = run_curator_review(on_summary=on_summary)
            # Fork: same config value run_curator_review() just used for its own `consolidate is None`
            # branch — re-read here (cheap, config is cached) so the hot-tier audit call below stays in
            # lockstep with the skill pass without run_curator_review needing to return it.
            consolidate = get_consolidate()

            # Fork: hot-tier audit — opt-in, dry-run-first pass over MEMORY.md/USER.md for stale-path
            # candidates, optionally upgraded to LLM keep/demote/stale/dead classification. Runs after
            # the skill-curation logic above. Its own report goes to a separate file (see
            # hot_tier_audit._hot_tier_reports_root: a race with the skill curator's async report write
            # rules out sharing one file) but its summary folds into this result dict.
            # See docs/plans/2026-07-14-hot-tier-audit.md.
            if get_hot_tier_audit():
                try:
                    from agent import hot_tier_audit
                    # Pass consolidate explicitly (the same value the skill pass was gated on) rather
                    # than letting hot_tier_audit re-read config: one read per maybe_run_curator call.
                    audit_summary = hot_tier_audit.run_hot_tier_audit(
                        dry_run=get_hot_tier_audit_dry_run(), consolidate=consolidate,
                    )
                    ...
                    with contextlib.suppress(Exception):
                        ...
                        llm_bit = ""
                        if llm_info.get("ran"):
                            llm_bit = ("; llm classification ok" if llm_info.get("succeeded")
                                       else "; llm classification failed (no mutation)")
                        _notify(
                            on_summary,
                            f"curator: hot-tier audit checked {n_checked} entries, "
                            f"{n_stale} stale-path candidate(s){llm_bit}",
                        )
                except Exception as e:
