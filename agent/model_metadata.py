...
def _count_image_tokens(msg: Dict[str, Any], cost_per_image: int) -> int:
    ...
    # Anthropic stashes the full provider content array on assistant messages: the public
    # ``anthropic_content_blocks`` key (attached after each Anthropic response), with the
    # underscore-prefixed key as the older back-compat stash. Mirror the adapter's precedence
    # (public first, private fallback) so a message carrying both is not counted twice.
    count += _count_parts(msg.get("anthropic_content_blocks") or msg.get("_anthropic_content_blocks"), {"image"})
...
def _wire_message_shadow(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * When ``anthropic_content_blocks`` is present on an assistant message it IS what ships to
      the Anthropic API (``anthropic_adapter``: ``content = msg.get("anthropic_content_blocks")``
      with the visible ``content`` string as fallback). The string ``content`` on the same message
      is an extracted text duplicate, and so are ``reasoning`` / ``reasoning_content`` /
      ``reasoning_details`` (the interleaved-thinking fast path replays ``ordered_blocks`` verbatim
      and never reads them). Counting them all quadrupled the thinking block and drove the
      preflight estimate far past the provider's real ``prompt_tokens`` (~600K rough vs ~284K real).
      Treat blocks, when present, as the authoritative size and skip the duplicates.
    * The private ``_anthropic_content_blocks`` key is a distinct back-compat stash — always
      skipped, it never ships as-is.
    ...
    * Opaque provider blobs (``encrypted_content`` on codex reasoning / compaction items) are
    ...
    for k, v in msg.items():
        ...
        if k == "api_content":
            ...
            continue
        elif k == "anthropic_content_blocks":
            shadow[k] = [
                {"type": "image", "image": "[stripped]"}
                if isinstance(part, dict) and part.get("type") == "image" else part
                for part in v
            ] if isinstance(v, list) else v
        elif k in ("reasoning", "reasoning_content") and has_anthropic_blocks:
            # Duplicates of thinking text the blocks already carry for the provider (see docstring).
            continue
        elif k == "content" and has_anthropic_blocks:
            # Text-extracted duplicate of what the blocks ship.
            continue
