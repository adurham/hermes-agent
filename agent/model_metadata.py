...
def _wire_message_shadow(msg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    * The private ``_anthropic_content_blocks`` key (underscore-prefixed) is a
      distinct back-compat stash some callers use (see
      ``agent/anthropic_adapter.py``'s "Back-compat: some callers stash blocks
      under a private key" fallback) — always skipped, it never ships as-is.
    * Base64 image payloads are replaced with a placeholder; they are charged
    ...
    for k, v in msg.items():
        ...
        if k in ("reasoning", "reasoning_content"):
            ...
            continue
        if k == "content":
            # Skip the text-extracted duplicate when either the sidecar or
            # anthropic_content_blocks already represents the same content
            # for the provider.
            if sidecar_wins or has_anthropic_blocks:
                continue
            if isinstance(v, list):
                cleaned = []
                for part in v:
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    tool_tokens = 0
    if tools:
        tool_tokens = _estimate_tools_tokens_rough(tools)
    return ((total_chars + 3) // 4) + image_token_credit + tool_tokens
