...
def _commit_tool_result(
    ...
    *,
    ...
    persisted_result = _append_fork_reminder_hints(agent, ref, persisted_result)
...
def _append_fork_reminder_hints(agent, ref: _ToolCallRef, persisted_result):
    """FORK: append the fork's per-tool-call reminder hints to a committed tool result.

    Three best-effort nudges (never break tool execution), plus the recent-tool-args window
    the memory-recall reminder reads for query-candidate extraction:

    * skill-recall  — records ``skill_view`` loads, then hints when a loaded skill's content
      has scrolled out of context (``agent/fork``-side bookkeeping).
    * memory-recall — closes the warm-tier visibility gap: the agent rarely calls
      ``memory(action='recall', ...)`` unprompted, especially during hypothesis formation.
    * consult nudge — points at the ``consult`` tool after a run of risky calls; resets when
      the agent calls consult on its own.

    Lives here because upstream's Sep-2026 decomposition funnels BOTH the concurrent and the
    sequential path through ``_commit_tool_result``; the fork previously carried one copy of
    this block per path.
    """
    ...
    def _append(hint):
        ...
        if not hint:
            return
        if _is_multimodal_tool_result(persisted_result):
            _append_subdir_hint_to_multimodal(persisted_result, hint)  # type: ignore[arg-type]
        else:
            persisted_result += hint

    try:
        if name == "skill_view":
            agent._record_loaded_skill(args.get("name", ""), persisted_result)
        _append(agent._maybe_skill_recall_hint(name))
        _append(agent._maybe_memory_recall_hint(name))
        if name == "consult":
            agent._record_voluntary_consult()
        _append(agent._maybe_consult_nudge(name))
    except Exception:
        logger.debug("fork reminder hints failed for %s", name, exc_info=True)
    # Sliding window of recent tool args (3-deep) for the memory-recall extractor.
    try:
    ...
    def _execute(next_args: dict) -> Any:
        ...
        with model_tools.suppress_post_tool_call_hook():
            return model_tools.handle_function_call(
                ...
                # FORK: parity with invoke_tool's concurrent dispatch — handle_function_call
                # takes an optional agent ref (CC-alias adaptation, tool-search stickiness).
                agent=agent,
            )
