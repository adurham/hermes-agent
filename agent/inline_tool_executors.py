...
def _memory(agent, args: dict, ctx: InlineToolContext) -> Any:
    result = _call_tool(
        ...
        (
            ...
            # Warm-tier args (fork) — must be forwarded so recall / recall_related /
            # read / replace / remove / feedback / promote / demote work. Without
            # these the warm dispatcher in tools/memory_tool.py rejects valid calls
            # with "query is required for recall." etc.
            ("tier", "tier", "hot"), ("query", "query"), ("top_k", "top_k"),
            ("category", "category"), ("tags", "tags"), ("fact_id", "fact_id"),
            ("helpful", "helpful"),
        ),
        store=agent._memory_store,
        # Agent ref — used by warm recall to reset the memory-recall-reminder
        # counter on voluntary calls.
        agent=agent,
    )
