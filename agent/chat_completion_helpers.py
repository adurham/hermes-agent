...
def _build_chat_completions_kwargs(agent, api_messages, tools_for_api, reasoning_config, request_overrides, cache_scope_id):
    ...
    # FORK: Anthropic-compatible max-output fallback (last resort only — applied in
    # build_kwargs *after* ephemeral/user/profile max_tokens, never overriding an explicit
    # value). Model-gated, not URL-gated: any chat-completions proxy serving a
    # Claude/MiniMax/Qwen3 model needs max_tokens, because the Anthropic Messages API
    # treats it as mandatory and proxies that omit it (AWS Bedrock, NVIDIA, LiteLLM, vLLM,
    # corporate gateways) default as low as 4096 output tokens — easily exhausted by
    # thinking + large tool calls like write_file/patch. Model-gated on
    # _ANTHROPIC_OUTPUT_LIMITS membership covers every aggregator route, not just
    # OpenRouter/Nous.
    _ant_max = None
    with contextlib.suppress(Exception):
        from agent.anthropic_adapter import _ANTHROPIC_OUTPUT_LIMITS, _get_anthropic_max_output
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    if raw_reasoning_content is not None:
        ...
        # FORK: exo omits the pad entirely (see message_sanitization.omits_reasoning_pad_for_provider).
...
class _StreamingCall(StreamingWaitMonitor):
    ...
    def __init__(self, agent, api_kwargs: dict, on_first_delta):
        ...
        # FORK: SSE ping visibility (Anthropic wire only — see the observer installed in
        # _call_anthropic). Pings prove "connection alive, server still working" during a long
        # cold start, and the cadence lets a heartbeat tell real thinking from a wedged socket.
        ...
        self._stream_stale_timeout_base = None
    ...
    @property
    def _stream_stale_timeout(self):
        """FORK: cold-start grace on top of the resolved base. Before the first stream event a
        slow provider (large prompt, queue stall, cold prefill) must not be killed at the
        mid-stream threshold; once events flow, silence IS a real stall. Exposed as a property
        so the (upstream-owned) monitor loop, the socket read timeout and the stale-kill path
        all read the effective value without upstream needing to know about the split. Pure
        computation lives in agent/fork/stream_recovery.effective_stale_timeout.

        A direct assignment (``self._stream_stale_timeout = N``, the pre-fork/upstream shape
        of this attribute) sets the override flag below and short-circuits the cold-start
        split entirely — the assigned value IS the effective timeout, no computation."""
        base = self._stream_stale_timeout_base
        if base is None:
            return None
        if getattr(self, "_stream_stale_timeout_override", False):
            return base
        from agent.fork.stream_recovery import effective_stale_timeout
        return effective_stale_timeout(self.first_event_seen["yes"], base)

    @_stream_stale_timeout.setter
    def _stream_stale_timeout(self, value):
        self._stream_stale_timeout_base = value
        self._stream_stale_timeout_override = True
    ...
    def _call_chat_completions(self, stream_attempt_id: int):
        ...
        for chunk in _iter_provider_stream_chunks(stream, response=lambda: self._attempt_stream_response):
            self.first_event_seen["yes"] = True  # FORK: ends the cold-start grace window
    ...
    def _call_anthropic(self, request_client):
        ...
        def _open_anthropic_stream(next_api_kwargs: dict[str, Any]):
            ...
            # FORK: prefer ``.beta.messages.stream`` when the client exposes it — the fork's
            # Claude-Code-mimicry path attaches beta-only typed kwargs (context_management,
            # output_config, speed, betas) that plain ``.messages.stream`` rejects with TypeError.
            # Fall back to ``.messages`` for clients without a ``.beta`` namespace, stripping the
            # beta-only kwargs first.
            _beta = getattr(request_client, "beta", None)
            if getattr(_beta, "messages", None) is not None:
                _stream_api = _beta.messages
            else:
                from agent.anthropic_adapter import _BETA_ONLY_KWARGS
                for _k in _BETA_ONLY_KWARGS:
                    final_kwargs.pop(_k, None)
                _stream_api = request_client.messages
            manager = _stream_api.stream(**final_kwargs)
        ...
        def _anthropic_stream_created(raw_stream: Any) -> None:
            ...
            # FORK: capture anthropic-ratelimit-* headers from the initial HTTP response so the
            # streaming heartbeat can tell "stalled near a rate limit" from "pure upstream queue".
            self._quiet(lambda: self.agent._capture_rate_limits(response))
            self._writer_token = claim_stream_writer(self.agent)
            self._reabort_if_cancelled(response)

        # FORK: install the SSE ping observer BEFORE the stream opens so the stale-stream
        # detector sees Anthropic's keep-alive pings during cold start (the SDK drops ``ping``
        # events before they reach the high-level iterator, so a long queue/prefill phase looks
        # identical to a black-holed connection without this).
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    set_sse_event_callback(None)  # FORK: uninstall the ping observer
    ...
    def _handle_stream_error(self, e: Exception, attempt: int, max_retries: int) -> bool:
        ...
        if self.deltas_were_sent["yes"]:
            ...
            _partial_tool_in_flight = bool(self.result.get("partial_tool_names")) or self.provider_tool_in_flight["yes"]
            # FORK: an upstream server that DELIBERATELY failed the turn on an unparseable
            # tool-call block is independently retryable even with no partial tool NAME parsed:
            # nothing executed (the call never parsed) and the garbled innards are never
            # surfaced, so re-streaming is side-effect-safe and strictly better than a dead
            # length-truncated stub.
            _is_toolcall_clean_fail = self.agent._is_upstream_toolcall_clean_fail(e)
            if not (((_partial_tool_in_flight and _is_transient) or _is_toolcall_clean_fail)
    ...
    def _resolve_stale_timeout(self) -> None:
        ...
        if base == 180.0 and self.agent.base_url and is_local_endpoint(self.agent.base_url):
            # FORK: the local ceiling lives in a helper so the stale detector and the Responses
            # first-event watchdog share it; assign the BASE (the ``_stream_stale_timeout``
            # property applies the cold-start grace on top).
            self._stream_stale_timeout_base = _local_stream_stale_timeout_default()
            logger.debug("Local provider detected (%s) — stale stream timeout set to %.0fs",
                self.agent.base_url, self._stream_stale_timeout)
            return
        self._stream_stale_timeout_base = _cloud_stale_timeout_for(self.agent, self.api_kwargs)
