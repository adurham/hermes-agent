...
def interruptible_streaming_api_call(
    ...
    def _call_anthropic(request_client):
        ...
        try:
            # Use the Anthropic SDK's streaming context manager. Uses
            # ``request_client`` (a per-request Anthropic client registered
            # with the stranger-thread abort machinery) rather than the
            ...
            with request_client.messages.stream(**api_kwargs) as stream:
                ...
                if not saw_stream_event and (
                    _final is None
                    or (
                        not getattr(_final, "content", None)
                        and getattr(_final, "stop_reason", None) is None
                    )
                ):
                    raise EmptyStreamError(
