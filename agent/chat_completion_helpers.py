...
def build_api_kwargs(agent, api_messages: list, tools_for_api: list | None = None) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        anthropic_kwargs = _transport.build_kwargs(
            ...
            # getattr-guarded like the neighbors: plugin engines / test doubles
            # without the fork's deferral builder just skip tool deferral.
...
def interruptible_streaming_api_call(
    ...
    def _call_anthropic(request_client):
        """Upstream relay_llm.stream path for the Anthropic Messages API."""
        ...
        def _anthropic_stream_created(raw_stream: Any) -> None:
            ...
            # HTTP response. Lets the streaming heartbeat answer "is this stall
            # near a rate limit, or pure upstream queue?".
            try:
                agent._capture_rate_limits(getattr(raw_stream, "response", None))
            except Exception:
        ...
        # Install SSE ping observer before the stream is opened so the
        # stale-stream detector sees keep-alive pings during cold start.
        ...
        try:
            ...
            try:
                for event in stream:
                    ...
                    # it arrives — proves the request was accepted and tells
                    # the user how big the prompt was + how much was cached.
                ...
                if not agent._interrupt_requested:
                    ...
                    if raw_stream is not None:
                        try:
                            ...
                            raise
                        # Capture routing headers before the context manager exits.
                        try:
                            _http_resp = getattr(raw_stream, "response", None)
                            _hdrs = getattr(_http_resp, "headers", None) if _http_resp else None
                            if _hdrs and base_final_message is not None:
                                _rid = _hdrs.get("request-id") or _hdrs.get("x-request-id")
                                if _rid:
                                    try:
                                        object.__setattr__(base_final_message, "_hermes_request_id", _rid)
                                    except Exception:
                                        pass
                                ...
                                if _routing:
                                    try:
                                        object.__setattr__(base_final_message, "_hermes_routing_headers", _routing)
                                    except Exception:
                                        pass
                        except Exception:
                            pass
                        # Stream lifecycle summary for historical evidence.
                        try:
                            _now = time.time()
        ...
        if _hb_now - _last_heartbeat >= _HEARTBEAT_INTERVAL:
            ...
            if _user_elapsed >= int(_HEARTBEAT_INTERVAL):
                try:
                    ...
                    # Recovery ETA: tells the user WHEN auto-reconnect kicks
                    # in if the stall is a genuine stale connection, not just
                    # that we're waiting. Folded into the diagnostic suffix
                    # (rather than a separate _emit_wait_notice call) so the
                    # rich stream-lifecycle evidence and the recovery timer
                    # land in the SAME line instead of two competing status
                    # writers racing to own the live spinner text.
                    if (
                    ...
                    if thinking_active["yes"] and _thinking_delta_chars > 0:
                        ...
                        # Also rewrite the live spinner/status line (CLI/TUI/
