...
def _summarize_user_message(user_message) -> str:
    """Text worth titling: describe a ``/skill`` invocation (it embeds the whole skill body), then strip wrappers.

    FORK: content-block lists / dicts (the CLI's shape when the user attached files or images)
    are coerced to text by ``_extract_text`` FIRST. Without this the whole function raises
    ``AttributeError`` on ``.strip()``, and the fork's image-stripping guard — which keeps a 339KB
    base64 screenshot out of the title prompt and off Anthropic's 200K ceiling — is dead code.
    """
    if not user_message:
        return ""
...
def generate_title(
    ...
    # FORK: one rate-limit retry (honoring Retry-After) before giving up.
    max_retries = 1
    for attempt in range(max_retries + 1):
        try:
            ...
            # FORK: rate-limit handling. Title generation is a background
            # convenience; tight per-minute quotas (e.g. Code Assist) must not
            # produce a prominent UI warning, and one honoring-Retry-After retry
            ...
            if is_rate_limit and attempt < max_retries:
                ...
                logger.info(
                    "Title generation rate limited. Retrying in %ss (attempt %d/%d)",
            ...
            if is_rate_limit:
                # Quiet skip: no failure_callback, no WARNING.
                logger.info("Title generation skipped (rate limited): %s", e)
            else:
                # WARNING so it shows in agent.log without debug mode.
