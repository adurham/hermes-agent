...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "tools, or capabilities, the documentation at https://github.com/adurham/hermes-agent/tree/main/website/docs is your "
...
HERMES_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Hermes Agent (by Nous Research). When the user needs help with Hermes itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://github.com/adurham/hermes-agent/tree/main/website/docs is the "
...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "Tool use supports your answer — it never replaces it. When the user asks for code, a fix, or other "
    "content to be returned in your reply (e.g. 'return the corrected class in a ```python``` block'), your "
    "final response must contain that content inline, in exactly the requested format. Writing it to a file "
    "or summarizing the changes you made does NOT fulfill such a request — and for a small, self-contained "
    "answer the user asked to see, prefer answering inline over creating files at all."
)
...
def _render_skills_index(
    ...
    return (
        ...
        "Loading a skill is NOT a one-time act: its content scrolls out of attention as the session continues. "
        "Before each destructive operation (cluster restart, file overwrite, bench launch, mass-edit, "
        "ssh ... pkill, anything that could break things) call `skill_pitfalls(name)` to re-surface that "
        "skill's gotchas section cheaply (~500-3000 tokens, vs ~30k for skill_view). The system will also "
        "periodically remind you to do this via a hint appended to tool results, but you should reach for it "
        "proactively — the recurring failure mode is 'I loaded the skill so I know what's in it' when in "
        ...
        "Hermes Agent itself — its CLI, config, models, providers, tools, skills, voice, gateway, plugins, or "
        "any feature — load the `hermes-agent` skill first. It has the actual commands (e.g. "
        "`hermes config set …`, `hermes tools`, `hermes setup`) so you don't have to guess or invent "
        "workarounds.\n"
