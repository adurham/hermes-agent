...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            ...
            self._write(f"\r{line}{' ' * max(self.last_line_len - line_width, 0)}", end='', flush=True)
...
def _cute_todo_checklist_lines(result) -> list:
    """Fork: render each todo item as an indented status line under the header.

    ``todo_tool()`` returns the complete item list on every call (read,
    create, or merge-update) -- see tools/todo_tool.py -- so the same payload
    the desktop/TUI clients render as a checklist is available here too.
    Without this the terminal only ever showed a bare "N task(s)" count.

    completed/in_progress/cancelled markers match the post-compression
    injection format in TodoStore.format_for_injection. "pending"
    intentionally diverges: format_for_injection's target is the model's
    context (glyph choice doesn't matter there), while this renders to the
    user's terminal, where WHITE LARGE SQUARE (U+2B1C) is
    Emoji_Presentation=Yes and gets drawn as a solid white block by
    color-emoji fonts regardless of the active theme -- unreadable on a dark
    terminal. Plain ASCII "[ ]" can never be emoji-rendered and matches the
    universal Markdown checklist convention, so it is used here instead.
    """
    try:
        items = (safe_json_loads(result) or {}).get("todos") if result else None
    except Exception:
        return []
    if not isinstance(items, list) or not items:
        return []
    markers = {"completed": "\u2705", "in_progress": "\U0001f504",
               "pending": "[ ]", "cancelled": "\u274c"}
    max_shown = 30
    lines = []
    for item in items[:max_shown]:
        ...
        lines.append(f"      {marker} {_tail_trunc(str(item.get('content', '')), 70)}")
...
def _get_cute_tool_message(tool_name: str, args: dict, duration: float, result: str | None = None) -> str:
    ...
    if tool_name == "todo_list" and not is_bridge:
        checklist = _cute_todo_checklist_lines(result)
        if checklist:
            return "\n".join([line] + checklist)
    return line
