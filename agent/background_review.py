...
def _run_review_in_thread(
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # Together these let (a) the parent's interrupt()/Ctrl+C cancel a
            # still-running review — the fork is a fully separate AIAgent with
            # its own _interrupt_requested flag that nothing otherwise fans out
            # to — and (b) the NEXT live turn proactively cancel an unfinished
            # review rather than letting the two race concurrently against the
            # same session_id/credentials, which produced doubled prompt-token
            # accounting and a Ctrl+C-proof lockup (#67821).
            # Best-effort by design: an agent built without going through
            # agent_init.py's setup (test stubs, older/foreign AIAgent
            # construction paths) won't have these attributes yet — degrade to
            # "no cross-cancellation" rather than aborting the whole review,
            # matching this function's existing best-effort style everywhere
            # else.
            try:
