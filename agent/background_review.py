...
# FORK: iteration budget raised from upstream's 16 to 50.
...
def _resolve_review_runtime(agent: Any, task_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # FORK: delegate to the shared auxiliary resolver instead of reading
    # ``auxiliary.background_review`` straight off the raw config dict. The naive read only
    # understands the legacy TASK-FIRST schema, so a PROVIDER-FIRST config
    # (``auxiliary.<provider>.background_review``) was silently ignored and per-provider
    # overrides had no effect. ``_resolve_task_provider_model`` handles both schemas and is
    # what every other auxiliary task (curator, compression, vision, ...) already uses.
    try:
