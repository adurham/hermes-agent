...
def _memory_parts(agent: Any) -> List[str]:
    ...
    if agent._memory_store:
        for enabled, kind in ((agent._memory_enabled, "memory"), (agent._user_profile_enabled, "user")):
            ...
            if block:
                parts.append(block)
        # FORK: warm-tier status — a one-line "WARM MEMORY: N facts indexed" block teaching
        # the agent that on-demand recall is available. None when the warm tier is empty or
        # unavailable; safe every turn (a fast SQLite COUNT(*)).
        if agent._memory_enabled or agent._user_profile_enabled:
            try:
                warm_block = agent._memory_store.format_for_system_prompt("warm_status")
                if warm_block:
                    parts.append(warm_block)
            except Exception:
                pass
        # FORK: session-pinned facts — warm-tier facts pinned for the rest of this session.
        # None when nothing is pinned; best-effort, a render failure skips the block rather
        # than crashing prompt assembly. See agent/fork/memory_session_pin.py.
        try:
