...
class InterruptControlMixin:
    ...
    def interrupt(
        ...
        Note on ``_pending_redirect``: a prior ``redirect()`` call may have already been accepted (the
        surface confirmed it, e.g. the CLI's "Redirected current turn") but not yet drained by the
        conversation loop — the loop only consumes ``_pending_redirect`` once per iteration, at the top
        of the tool-calling while-loop. If a stop lands in that window it must not silently destroy that
        accepted, undelivered correction. Instead it is folded into ``message`` below so it rides out on
        the normal ``_interrupt_message`` -> ``result["interrupt_message"]`` -> next-turn requeue path.

        Note on ``_pending_steer``: the same problem applies to an accepted ``steer()`` correction (a
        plain /steer, or a redirect() that degraded to steer() because the agent was mid-tool-call). It
        is only consumed once, by the tool-batch drain hook, which won't run if the stop lands first.
        Folded below, same as the redirect, rather than silently discarded.
        """
        # ``message`` is typed Optional[str], but at least one caller path (CLI's
        # queued-Enter-with-images payload, ``(text, [Path, ...])``) has been observed
        # handing a multimodal tuple straight through to this method — crashing the
        # interrupt path itself (``TypeError: sequence item 0: expected str instance,
        # tuple found``) instead of actually interrupting. Coerce defensively here so
        # every caller (gateway, tui_gateway, cron, CLI) gets a safe str regardless of
        # what slipped through; the image half of a multimodal payload has no home on
        # this text-only channel and is dropped (callers needing it preserved should
        # route the full payload through _pending_input instead).
        ...
        def _fold_dropped(_dropped_redirect, _dropped_steer):
            # Preserve an accepted-but-undelivered redirect and/or steer correction by merging it
            # into the interrupt message instead of discarding it.
        ...
        def _publish_interrupt_state(_folded_message) -> None:
        ...
        with _ic_lock(self, "_pending_redirect_lock"):
            ...
            # Nested inside the redirect lock (never the reverse anywhere else in this module) so no
            # other call path can deadlock.
            _dropped_steer = _drain_steer_locked()
            _folded_message = _fold_dropped(
                _ic_slot(self, "_pending_redirect_lock", "_pending_redirect"), _dropped_steer
            )
            if require_generation is None:
                ...
                _publish_interrupt_state(_folded_message)
            else:
