...
def _build_switched_client(agent, new_provider, api_key, base_url, api_mode, new_norm) -> None:
    ...
    if api_mode == "anthropic_messages":
        ...
        # FORK: re-evaluate the 1M-beta latch for the new model — switching to a non-1M model
        # (Opus -> Haiku) sets it so the new client does not carry the rejected beta; going the
        # other way clears it so 1M-capable models can use the beta again.
        try:
            from agent.anthropic_adapter import _model_supports_1m_context
            agent._oauth_1m_beta_disabled = not _model_supports_1m_context(agent.model)
        except Exception:
            pass
        agent._anthropic_client = build_anthropic_client(
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    from agent.message_sanitization import apply_reasoning_content_policy, omits_reasoning_pad_for_provider
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a strict provider that rejects the field (Mistral, Cerebras, Groq, SambaNova, …):
    ...
    """
    from agent.message_sanitization import reapply_reasoning_echo, omits_reasoning_pad_for_provider
