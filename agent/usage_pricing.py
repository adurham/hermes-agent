...
for _provider, _url, _version, _rows in _SNAPSHOTS:
    for _models, _rates in _rows.items():
        ...
        # FORK: Anthropic bills cache writes per TTL — 5m at the base cache-write rate, 1h at
        # 1.6x that (== 2x the input rate), per
        # https://platform.claude.com/docs/en/about-claude/pricing. The snapshot table above
        # carries only the single legacy rate, so derive the split here rather than duplicating
        # every row. Entries with no cache-write rate (e.g. token-only providers) are untouched.
        if _provider in ("anthropic", "bedrock") and _entry.cache_write_cost_per_million is not None:
            _entry = replace(
                _entry,
                cache_write_5m_cost_per_million=_entry.cache_write_cost_per_million,
                cache_write_1h_cost_per_million=_entry.input_cost_per_million * 2,
            )
...
def estimate_usage_cost(
    ...
    _unsplit_cache_write = max(0, usage.cache_write_tokens - _split_tokens)
    _has_split_rates = (
        entry.cache_write_5m_cost_per_million is not None or entry.cache_write_1h_cost_per_million is not None
    )
    # Unsplit tokens fall back to the legacy single rate, then 5m, then 1h — in that priority
    # order — so historical sessions (written before the breakdown landed) keep their old totals
    # and no tokens are ever silently dropped from the bill.
    _cache_write_rate = entry.cache_write_cost_per_million
    if _cache_write_rate is None and _has_split_rates:
        _cache_write_rate = entry.cache_write_5m_cost_per_million or entry.cache_write_1h_cost_per_million
    ...
    for tokens, rate, rate_above, note in (
        ...
        (_unsplit_cache_write, _cache_write_rate, entry.cache_write_cost_per_million_above,
         ("cache-write pricing unavailable for route",)),
        (usage.cache_write_5m_tokens, entry.cache_write_5m_cost_per_million, None, ()),
        (usage.cache_write_1h_tokens, entry.cache_write_1h_cost_per_million, None, ()),
    ):
