...
class _ThreadRoutingStream:
    ...
    def fileno(self):  # type: ignore[no-untyped-def]
        ...
        # lifetime (see module docstring + ``_sinks``) and should never
...
def thread_scoped_silence() -> Iterator[None]:
    ...
    try:
        ...
        # Sinks are process-lifetime (see ``_sinks`` / ``_RoutingState``) —
        # never closed here; closing per-call was the original bug.
