...
# Fork naming: MCP tools are exposed as ``<server>_<tool>`` (the ``mcp_``/``mcp__`` prefix is
# dropped — see tools/mcp_tool.py::mcp_prefixed_tool_name), so the filesystem entries below are
# unprefixed. skill_view/skills_list are upstream additions (v2026.9.14).
IDEMPOTENT_TOOL_NAMES = frozenset({
    "read_file", "search_files", "web_search", "web_extract", "session_search", "skill_view", "skills_list",
    "browser_snapshot", "browser_console", "browser_get_images", "filesystem_read_file",
    "filesystem_read_text_file", "filesystem_read_multiple_files", "filesystem_list_directory",
    "filesystem_list_directory_with_sizes", "filesystem_directory_tree", "filesystem_get_file_info",
...
class ToolCallGuardrailConfig:
    """Thresholds for per-turn tool-call loop detection. Warnings never prevent execution.

    FORK: ``hard_stop_enabled`` defaults to True (upstream ships False + auto-on for unattended
    platforms only). See FORK.md "Fork-only fix — 2026-07-07 (tool-call loop guardrails now block
    by default)": warn-only guardrails were routinely ignored by the model. Opt out with
    ``tool_loop_guardrails.hard_stop_enabled: false`` in config.yaml. Upstream's
    ``non_interactive_hard_stop_enabled`` escalation is kept and is a no-op while this is True."""
