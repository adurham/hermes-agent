...
class SessionDB(
    ...
    _CONVERSATION_ROW_COLUMNS = (
        ...
        # FORK: Anthropic-native content blocks (thinking-signature fidelity). Appended LAST so
        # upstream positional decoding is unaffected; ALTER-ADDed by the FORK_TABLE_COLUMNS pass
        # in SessionDB._reconcile_columns. See FORK.md T2.4.
        "anthropic_content_blocks"
    )

    # ── Fork-only additions (see FORK.md) ────────────────────────────────
    # Re-homed here during the v2026.9.14 decomposition: upstream moved these
    # neighbourhoods into hermes_state_* mixins, which are pure-upstream. These
    # stay on SessionDB itself so the MRO resolves them ahead of the mixins and
    # the fork surface never has to edit an upstream-owned module.

    def _reconcile_columns(self, cursor: sqlite3.Cursor) -> None:
        """Ensure live tables have every column declared in SCHEMA_SQL,
        plus the fork's own extra columns.

        Delegates the upstream reconciliation (SCHEMA_SQL-declared columns)
        to ``SessionSchemaMixin._reconcile_columns`` via ``super()`` so the
        two implementations never duplicate/diverge. Before that, ensures
        FORK_SCHEMA_SQL's own tables (e.g. ``api_calls``) exist -- this must
        run before any reconciliation touches them. After the upstream pass,
        merges FORK_TABLE_COLUMNS into the expected-columns set and ALTER-ADDs
        any fork-only columns (e.g. ``anthropic_content_blocks``) the same
        way, without ever declaring them inside SCHEMA_SQL's upstream-owned
        CREATE TABLE bodies (keeps them conflict-free on future syncs).
        """
        # Shape heal (was the v12→v13 version-gated migration in the
        # monolithic layout): api_calls' session_id FK must carry ON DELETE
        # CASCADE or prune_sessions' retention sweep fails with a FOREIGN KEY
        # violation on any session that has telemetry rows. SQLite can't
        # ALTER a foreign key, so a legacy no-CASCADE table is dropped and
        # recreated by the FORK_SCHEMA_SQL pass below (CREATE IF NOT EXISTS).
        # Per-call telemetry is cheap to lose — session-level cumulative
        # counts live on the sessions table. Self-healing (probe the live
        # FK shape) rather than version-gated, mirroring the upstream
        # _heal_*_pk repairs, so it also fixes DBs whose version already
        # advanced past the gate.
        try:
            _fk_rows = cursor.execute(
                "PRAGMA foreign_key_list(api_calls)"
            ).fetchall()
            for _fk in _fk_rows:
                _from = _fk[3] if isinstance(_fk, (tuple, list)) else _fk["from"]
                _on_delete = (
                    _fk[6] if isinstance(_fk, (tuple, list)) else _fk["on_delete"]
                )
                if _from == "session_id" and str(_on_delete).upper() != "CASCADE":
                    cursor.execute("DROP TABLE IF EXISTS api_calls")
                    break
        except sqlite3.OperationalError:
            pass  # table absent — FORK_SCHEMA_SQL creates it fresh
        cursor.executescript(FORK_SCHEMA_SQL)
        super()._reconcile_columns(cursor)
    ...
    def get_compression_attempts_total(self, session_id: str) -> int:
        ...
        if not session_id:
            return 0
        # Pure read — route through the pooled WAL reader so it never
        # convoys every concurrent turn's persistence behind the writer
        # lock (Pattern C; see _read_ctx). The writer commits its counter
        # increment inside _execute_write before any read, so the read
        # sees committed data and needs no read-your-writes ordering.
        with self._read_ctx() as conn:
            row = conn.execute(
    ...
    def get_lineage_cost_usd(self, session_id: str) -> float:
        ...
        root = session_id
        for _ in range(100):
            # Pure read — pooled WAL reader, no writer-lock convoy.
            with self._read_ctx() as conn:
                cursor = conn.execute(
