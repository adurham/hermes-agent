...
# FORK: thin forwarders for fork-only features (agent/fork/* is the hard-fork boundary).
# Listed LAST in the AIAgent bases below so upstream's extracted mixins keep priority for
# the one name both define (``_decorate_xai_entitlement_error``), preserving pre-merge
# fork behavior where AIAgent's own class-body copy shadowed the forwarder.
...
def _fork_memory_session_end(agent: Any, messages: Optional[list]) -> None:
    """FORK: session-end passes for the fork-only warm-memory layers (Phase 2 auto-extraction,
    Phase 3 auto-feedback). Independent of the upstream memory-manager / context-engine
    shutdown above; both are no-ops unless ``memory.auto_extract`` / ``memory.auto_feedback``
    are enabled in config. Best-effort — teardown must never raise into the agent loop.

    The CLI's session-finalize callback surfaces the confirm UI; here the extractor just
    stashes proposals back to the buffer for the next interactive session.
    """
    try:
        from tools import memory_extraction as _mex
        _summary = _mex.on_session_end(agent.session_id or "", messages or [], interactive=False)
        if _summary.get("final_proposed", 0) or _summary.get("committed", 0):
            logger.info(
                ...
                _summary.get("buffered", 0), _summary.get("final_proposed", 0),
                _summary.get("committed", 0), _summary.get("skipped", 0),
            )
    except Exception:
        pass
    # Drop the per-session recall window so a long-running CLI process doesn't accumulate
    # session state forever.
    try:
        from tools.memory_auto_feedback import flush_session as _maf_flush
        _maf_flush(agent.session_id or "")
    except Exception:
        pass


def _fork_memory_turn_end(agent: Any, original_user_message: Any, final_response: Any) -> None:
    """FORK: per-turn hooks for the fork-only warm-memory layers.

    Phase 2 auto-extraction and Phase 3 auto-feedback (walk this session's recall window, match
    fingerprints against the assistant response, upvote any fact the assistant actually cited).
    Deliberately OUTSIDE the upstream ``sync_all``/``queue_prefetch_all`` try block (a provider
    error must not skip them) and outside its empty-text early return — fork HEAD ran these even
    when the flattened text was empty. Both no-op when their config flags are off; never blocks.
    """
    try:
        from tools import memory_extraction as _mex
        _mex.on_turn_end(
    ...
    try:
        ...
        _maf.on_turn_end(agent.session_id or "", assistant_text=str(final_response))
    except Exception:
        pass
...
class AIAgent(
    ...
    ForkForwardersMixin,  # FORK: last — upstream mixins win the one shared name (see import note)
):
    ...
    def __init__(
        ...
        # FORK: force one tool call per turn so the model emits a fresh <think> block before each
        # tool. For models that contractually emit one <think> per turn but chain multi-step
        # reasoning across turns (DSv4-Flash, etc). Threaded to agent_init via **init_kwargs.
    ...
    def _is_upstream_toolcall_clean_fail(self, error: BaseException) -> bool:
        """FORK: True for an upstream server that DELIBERATELY failed a turn because the model emitted a
        tool-call block it could not parse.

        exo's DSv4 output parser cleanly fails such a turn with ``finish_reason="error"`` (-> 500 /
        APIError) rather than leaking the garbled tool-call innards as content or silently dropping the
        call. These are SAFE to silently retry even though no partial tool *name* was parsed (so the
        normal ``_partial_tool_in_flight`` gate misses them): the failure happens server-side BEFORE any
        tool executes, so there is no side-effect to duplicate, and the garbled tool-call text is never
        surfaced, so there is no useful content to preserve. Called from
        ``agent.chat_completion_helpers``.
        """
        message = str(error).strip().lower()
        if "failing the turn so it can be retried" not in message:
            return False
        # Guard to the known tool-call clean-fail shapes so an unrelated error that happens to echo the
        # phrase can't widen the retry surface.
        return any(
            sig in message
            for sig in (
    ...
    def shutdown_memory_provider(self, messages: list = None) -> None:
        ...
        _fork_memory_session_end(self, messages)
    ...
    def _sync_external_memory_for_turn(self, *, original_user_message: Any, final_response: Any, interrupted: bool,
        ...
        # Upstream's empty-text guard is a conditional rather than an early return so the fork's
        # Phase-2/3 turn-end hooks below still fire (fork HEAD ran them unconditionally here).
        if user_text and response_text:
            try:
                ...
                pass
        _fork_memory_turn_end(self, original_user_message, final_response)
    ...
    def apply_runtime_credential_update(
        ...
        """FORK: update auth on the live agent in place, without a full rebuild.

        For same-provider/same-routing/same-model turns where only the resolved
        ``api_key``/``base_url`` changed (the common case: an OAuth token refresh mid-session —
        Claude Code's ``~/.claude/.credentials.json`` is shared across every concurrent ``hermes``
        process, so one process refreshing invalidates another's cached token and its next call
        gets a real 401), the caller previously discarded the whole ``AIAgent`` and rebuilt from
        scratch (see ``HermesCLI._ensure_runtime_credentials``). That loses in-memory turn state
        (checkpoints, cached tool schemas, credential-pool refresh counters) and prints a
        disruptive "Initializing agent..." on every refresh for no reason.

        Reuses the same in-place client-rebuild machinery ``ClientLifecycleMixin._swap_credential``
        uses for a pool-driven mid-turn 401 recovery, entered from a raw ``(api_key, base_url)``
        pair instead of a pool ``entry`` (this path fires before a turn starts, so there is no
        failing pool entry to hand in). Returns True on success; the caller should fall back to a
        full rebuild (``self.agent = None``) on False, since an in-place swap can fail for reasons
        a fresh client wouldn't (a bad TLS override, an SDK edge case).
        """
        ...
        try:
            ...
            if self.api_mode == "anthropic_messages":
                ...
                # Upstream consolidated direct-client construction (1M-beta drop policy included)
                # into ClientLifecycleMixin._build_direct_anthropic_client; use it rather than
                # re-deriving the beta headers here.
                self._anthropic_client = self._build_direct_anthropic_client(
                    api_key, self._anthropic_base_url
                )
                self._is_anthropic_oauth = self._anthropic_oauth_flag(api_key)
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        return _delegate_task(
            ...
            model=function_args.get("model"), agent_type=function_args.get("agent_type"),
