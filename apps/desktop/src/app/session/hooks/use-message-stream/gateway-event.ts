    ...
    (event: RpcEvent) => {
      ...
      if (event.type === 'gateway.ready') {
        ...
        if (isActiveEvent) {
          ...
          // `finalText` (already computed above for the message store) doubles
          // as "what just happened" context for the optional LLM-generated
          // pet-dialogue line — capped server-side, so no truncation needed here.
          triggerPetTurnCompleted(finalText)
