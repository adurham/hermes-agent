...
describe('useMessageStream composed with the real useSessionStateCache', () => {
  ...
  it('measures the frame cost through the real view-sync rAF and adapts the next gap', async () => {
    ...
    // Mid-turn state: busy keeps subsequent view syncs on the deferred rAF
    // path (terminal/needing-input states flush synchronously instead). This
    // seed is the session's FIRST paint — a session switch — so the cache
    // flushes it synchronously (no rAF) and the flush below starts clean.
    act(() => {
