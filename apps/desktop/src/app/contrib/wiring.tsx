...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  const nextActions: WiringActions = {
    ...
    // otherwise `stack` keeps browser-tab semantics for a plain sidebar
    // click — an occupied main stays put and the session opens as an
    // additional tab beside it, while an empty main (fresh draft, or a
    // full-page route) is loaded directly. The clicked ROW is the identity,
    // not its bare id: two profiles can hold twins with the same stored id
    // (#92454), and an id-only resume resolves against whichever cached row
    // is found first — the user clicks a row previewing profile A and the
    // resume dials profile B. Pin the row's own (connection, profile) as the
    // resume owner before navigating; untagged rows (single-profile installs
    // and the legacy primary-SSH path) keep the ambient/id-only path. Clear
    // any stale explicit hint first: older builds incorrectly persisted
    // those rows as `local`, which made a remote session click switch to the
    // Mac backend and fail with "session not found".
