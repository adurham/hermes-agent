  ...
  useEffect(() => {
    ...
    setSessionTileDelegate({
      ...
      resumeTile: async (storedSessionId, options) => {
        ...
        const storedRowPromise = resolveStoredSession(storedSessionId).catch(() => undefined)

        // Falls through here only when refreshTranscript was true (the warm
        // no-REST path above already returned otherwise): merge the REST
        // prefetch into the cached transcript instead of a full resume — an
        // explicit reopen must still catch deliveries that landed while the
        // panel's WS was down (#96183), which never arrive as realtime events.
        ...
        const prefetch = await prefetchPromise
        const storedRow = await storedRowPromise
