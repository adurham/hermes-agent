...
function renderTile(
  ...
  const updateSessionState = refs?.updateSessionState ?? vi.fn()
...
describe('useSessionTileDelegate resumeTile', () => {
  ...
  it('rejects a resume that comes back empty for a session with known history, instead of caching a blank tile', async () => {
    ...
    // A known default-profile owner resumes through the profile router, never
    // the ambient socket (#92961).
    vi.mocked(requestGatewayForProfile).mockResolvedValueOnce({ session_id: 'runtime-3', messages: [] } as never)
  ...
  it('accepts a genuinely empty resume for a brand-new session (message_count 0)', async () => {
    ...
    // A known default-profile owner resumes through the profile router, never
    // the ambient socket (#92961).
    vi.mocked(requestGatewayForProfile).mockResolvedValueOnce({ session_id: 'runtime-4', messages: [] } as never)
