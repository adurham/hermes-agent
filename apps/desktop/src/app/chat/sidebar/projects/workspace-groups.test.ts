...
describe('mergeRepoWorktreeGroups (manual lane order)', () => {
  it('uses the activity-based default order when no manual order is given', () => {
    const repo = {
      ...
      path: '/repo',
      groups: [
        lane({ id: 'main', label: 'main', isMain: true, sessions: [makeCwdSession('/x', { last_active: 1 })] }),
        lane({ id: 'busy', label: 'busy', isMain: false, sessions: [makeCwdSession('/x', { last_active: 500 })] }),
        lane({ id: 'stale', label: 'stale', isMain: false, sessions: [makeCwdSession('/x', { last_active: 10 })] })
      ]
    }
  ...
  it('surfaces a newly discovered lane ahead of a stale manual order instead of dropping it', () => {
    const repo = {
      ...
      groups: [
        lane({ id: 'main', label: 'main', isMain: true, sessions: [makeCwdSession('/x', { last_active: 1 })] }),
        lane({ id: 'a', label: 'a', isMain: false, sessions: [makeCwdSession('/x', { last_active: 5 })] }),
        lane({ id: 'b', label: 'b', isMain: false, sessions: [makeCwdSession('/x', { last_active: 5 })] })
      ]
    }
