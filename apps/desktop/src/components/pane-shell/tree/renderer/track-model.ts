...
interface PaneChrome extends PaneSizing {
   ...
   *  (activate) so that gesture survives, and the strip it lives in
   *  (own-slot exclusion + same-strip reorder target, mirrors
   *  `startPaneDrag`'s `reorder` param). Returns whether it took the
   *  drag; `false` (or absent) defers to `startPaneDrag` — e.g. the
