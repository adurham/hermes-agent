...
_INSERT_MESSAGE_SQL = """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   codex_message_items, anthropic_content_blocks, platform_message_id, observed, _compressed_summary, active, api_content, display_kind,
...
class SessionMessagesMixin:
    ...
    def _message_row_params(self, session_id: str, role: str, msg: Dict[str, Any], tool_calls: Any,
        ...
        *keep_reasoning* False NULLs every reasoning column). ``platform_message_id`` falls back to
        ``message_id`` (yuanbao's message-dict convention). ``anthropic_content_blocks`` (Claude
        thinking-signature replay fidelity) is fork-only and assistant-role-only."""
    ...
    def append_message(
        ...
        codex_message_items: Any = None, anthropic_content_blocks: Any = None, platform_message_id: str = None, observed: bool = False,
        ...
        it differed from ``content``, stored as sent except lone surrogates. ``anthropic_content_blocks``:
        the raw Claude content-block list (thinking-signature replay fidelity), assistant-role-only."""
            ...
            if row["role"] == "assistant":
                ...
                msg.update(
                    ...
                    for col in ("reasoning_details", "codex_reasoning_items", "codex_message_items",
                        "anthropic_content_blocks") if row[col])
