...
|---|---|---|
| `delegate_task` (`tools/delegate_tool.py`) | Parent spawns **child** `AIAgent` instances in-process; parent sees only the final summary. | No mechanism for two independently-started sessions (two terminal tabs, a CLI session and a gateway session, two profiles) to find and message each other — `delegate_task` children exist only inside one parent's single call, and there is no separate peer-messaging channel between them. (Note: `tools/swarm_tool.py`'s `swarm_run` and the optional external `hermes-swarm` package it wired mid-task peer messaging through were removed 2026-08-10 as unused; this row previously described that as an existing mechanism. `tools/swarm_board.py` — the live delegate_task progress-board UI — is unrelated and still in use.) |
