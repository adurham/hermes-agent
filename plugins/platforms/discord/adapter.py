...
def _format_privileged_intents_guidance(*, needs_members: bool) -> str:
    ...
    lines.extend(
        [
            ...
            "Docs: https://github.com/adurham/hermes-agent/blob/main/website/docs/user-guide/messaging/discord.md",
        ]
    )
    return "\n".join(lines)
...
class DiscordAdapter(DiscordMediaMixin, BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        # Fork-only: /submit fires a fire-and-forget run at the local api_server and
        # edits its own public reply when the run finishes. Not table-driven — it needs a
        # non-ephemeral defer + a background watcher, which _slash_proxy does not model.
        @tree.command(
            name="submit",
