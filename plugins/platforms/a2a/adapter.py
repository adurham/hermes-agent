...
def _orphan_timeout() -> float:
    ...
    Doubled against the reply window so a task that legitimately outlived one sync-RPC
    window (see _quick_ack_timeout / #78007 — the RPC hands back a pollable WORKING ack
    instead of failing) is not swept while the agent is still working.
    """
    return min(float(_MAX_ORPHAN_TIMEOUT), max(float(_MIN_ORPHAN_TIMEOUT), _reply_timeout() * 2))
...
class A2ARequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):  # noqa: N802
        ...
        if sec.localhost_only() or sec.authenticate(
                self.headers.get("Authorization"), self._client_ip(), self._forwarded_for()) is not None:
    ...
    def do_POST(self):  # noqa: N802
        ...
        # Identity comes from the credential (or the socket in localhost-only mode) — never the
        # body. ``_forwarded_for()`` only matters behind an allow-listed proxy (see #80534).
        identity = adapter._security_context.authenticate(auth_header, client_ip, self._forwarded_for())
        if identity is None:
            decision = (security.AUTH_REJECTED_BAD_TOKEN if token_fp
                        else security.AUTH_REJECTED_MISSING_TOKEN)
            return self._auth_reject(401, decision, None, protocol.ERR_UNAUTHORIZED, "unauthorized",
                                     identity=None, token_fp=token_fp, client_ip=client_ip)
        try:
        ...
        # request reaching the gate is audited, accepts included, as an intrusion signal).
        security.audit_auth(security.AUTH_ACCEPTED, status=200, source_ip=client_ip,
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _fail_orphans_once(self) -> list[str]:
        ...
        for tid in failed:
            # Drop the pending Future too: with #78007 the RPC path no longer pops it when
            # the quick-ack window closes, so the watchdog is the only thing left to reclaim it.
    ...
    def _record_outcome(self, task_id: str, context_id: str, peer: str, state: str, reply: str,
                        started: Optional[float] = None) -> Optional[tuple[str, str]]:
        """Persist + audit + count a finished task, mark it terminal, and fire its push callback.

        Marks the task terminal FIRST and gates every side effect on winning that
        transition. ``tasks.complete()`` holds the store lock across its read-check-write
        and returns None when the task was already terminal, so exactly one caller per
        task_id ever runs persist/audit/metrics/push. This matters because #78007's late
        Future done-callbacks (see _finalize_from_future) race the orphan watchdog for the
        same task_id — a separate pre-check would leave a window where both ran the side
        effects before the second ``complete()`` (correctly) no-opped.

        Returns the already-recorded ``(state, reply)`` when another caller won, else None.
        """
        if self.tasks.complete(task_id, state, reply) is None:
            existing = self.tasks.get(task_id)
            logger.debug("A2A: task %s already terminal; ignoring late outcome", task_id)
            return (existing["state"], existing.get("reply", "")) if existing else (state, reply)
    ...
    def _finalize_task(self, pending: dict, state: str, reply: str) -> tuple[str, str]:
        ...
        Idempotent and safe to call from a thread other than the request's — #78007 late
        Future done-callbacks run on the agent's event-loop executor. When another caller
        (e.g. the orphan watchdog) already made the task terminal, ``_record_outcome``
        returns that recorded outcome and skips re-persisting / re-auditing / re-pushing.
        """
        ...
        try:
            ...
            recorded = self._record_outcome(task_id, context_id, peer, state, reply, started=pending["started"])
            return recorded if recorded is not None else (state, reply)
        finally:
            self._pop_pending(task_id)

    @staticmethod
    def _await_future(fut: Future, deadline: float, keepalive,
                      on_timeout: Optional[tuple[str, str]]) -> tuple[str, str, bool]:
        """Block until ``fut`` resolves or ``deadline`` passes. ``keepalive`` runs every
        _SSE_KEEPALIVE seconds while waiting; if it raises, the client is gone and we stop.

        Returns ``(state, text, resolved)``. ``resolved`` is False only in the #78007
        quick-ack case — ``on_timeout=None`` and the deadline passed with the Future still
        pending: the caller MUST NOT finalize the task; it returns a non-terminal Task
        (WORKING) and leaves the pending Future in place so the eventual real result is
        still captured and retrievable via tasks/get. With an explicit ``on_timeout`` the
        timeout is terminal and that pair is returned as resolved.
        """
        while True:
            try:
                ...
                if time.time() >= deadline:
                    if on_timeout is not None:
                        return (*on_timeout, True)
                    # Deadline hit but the Future has NOT resolved. Return unresolved so the
                    # caller keeps the task non-terminal; the watchdog (bounded by
                ...
                return ((*on_timeout, True) if on_timeout is not None
                        else (protocol.STATE_FAILED, "[agent did not reply in time]", True))

    def _await_reply(self, pending: dict, keepalive=None) -> tuple[str, str, bool]:
        """Wait for the agent, bounded by ``_quick_ack_timeout()`` rather than the full
        ``_reply_timeout()`` window (#78007). A fast task still returns its real result
        synchronously the moment the Future resolves; a slow one hands back an early
        pollable WORKING ack instead of letting the caller's shorter HTTP timeout kill the
        socket before any response existed."""
        return self._await_future(pending["future"], pending["started"] + _quick_ack_timeout(),
                                  keepalive, None)

    def _deactivate_task(self, task_id: str) -> None:
        """Mark a task as no longer owned by an in-flight request while keeping its pending
        Future. Paired with the #78007 quick-ack: once the RPC has handed back a WORKING
        ack nobody is waiting on it, so the orphan sweep (which skips ``_active_tasks``)
        must be allowed to reclaim it if the agent never replies."""
        with self._pending_lock:
            self._active_tasks.discard(task_id)

    def _finalize_from_future(self, pending: dict, fut: "Future") -> None:
        """Future done-callback used when the sync-RPC window closed before the agent
        resolved. When the Future later fires, record the real outcome via the normal
        finalize path so tasks/get returns it."""
        try:
    ...
    def _rpc_message_send(self, req_id: Any, params: dict, peer: str, agent: Optional[dict] = None, v1_response: bool = False) -> dict:
        ...
        if task is None:
            ...
            if not resolved:
                # #78007: the sync-RPC window closed but the agent's Future is still pending.
                # Do NOT finalize as FAILED — leave the task WORKING so a real completion
                # arriving later is captured (done-callback -> _finalize_task) and stays
                # retrievable through tasks/get. Deactivate it so the watchdog can still
                # reclaim it; no request is waiting on it any more.
                pending["future"].add_done_callback(lambda f, p=pending: self._finalize_from_future(p, f))
                self._deactivate_task(pending["task_id"])
                state, reply = protocol.STATE_WORKING, ""
            else:
    ...
    def _rpc_message_stream(self, handler, req_id: Any, params: dict, peer: str, agent: Optional[dict] = None) -> None:
        ...
        try:
            ...
            state, reply, resolved = self._await_reply(pending, keepalive=self._keepalive(handler))
            if not resolved:
                # See _rpc_message_send: keep the task non-terminal and let the late Future
                # resolution finalize it. The SSE stream must still close, so emit a WORKING
                # status update with no done frame — unusual but valid; clients reconnect via
                # tasks/subscribe or poll tasks/get by task id.
                pending["future"].add_done_callback(lambda f, p=pending: self._finalize_from_future(p, f))
    ...
    def _rpc_tasks_subscribe(self, handler, req_id: Any, params: dict, agent: Optional[dict] = None) -> None:
        ...
        try:
            ...
            state, reply, _resolved = self._await_future(fut, time.time() + _reply_timeout(), self._keepalive(handler),
