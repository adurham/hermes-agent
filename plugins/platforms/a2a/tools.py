...
def _resolve_peer(agent: str, timeout_override: Optional[int] = None) -> Optional[dict]:
    ...
    if agent.startswith(("http://", "https://")):
        return {"url": agent, "auth": {}, "capabilities": [],
    ...
    if timeout_override:
        peer["timeout"] = int(timeout_override)
    return peer
...
_TOOLS: dict[str, tuple[Any, str, dict, list[str]]] = {
    ...
    "a2a_call": (a2a_call,
                 ...
                 {"agent": _str("Configured peer name (from a2a_agents) or a full http(s):// URL."),
                  ...
                  "timeout": {"type": "integer", "description": "Optional: seconds to wait for the peer's reply "
                              "(default 120). Raise for long-running peer tasks; the server's own reply window is "
                              "300s by default (A2A_REPLY_TIMEOUT)."}},
                 ["agent", "message"]),
