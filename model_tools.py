...
def get_tool_definitions(enabled_toolsets: Optional[List[str]] = None, disabled_toolsets: Optional[List[str]] = None,
                         ...
                         sticky_active: bool = False) -> List[Dict[str, Any]]:
    ...
    sticky_active (FORK): True when this conversation/agent has *ever* had
    tool_search activated (``agent._tool_search_ever_activated``). Keeps
    tool_search activated even if the live deferrable-token total has since
    dropped back under threshold — see ``tools/tool_search.py``
    ``assemble_tool_defs``: recomputing the activate/deactivate decision from the
    live global registry on every rebuild lets it flip mid-conversation when an
    unrelated MCP reconnect or subagent shifts the token total across the
    threshold, and Anthropic hard-rejects previous-turn tool_use blocks for names
    that vanish from the wire tools array. Does NOT change which tools are in the
    catalog — only whether the bridge tools are shown.
    """
    def compute():
...
def _tool_defs_cache_key(
    ...
    return (
        ...
        # FORK: sticky_active changes whether the tool_search bridge tools are in
        # the returned list, so it must be part of the memo identity.
        bool(sticky_active),
    )
...
def _compute_tool_definitions(enabled_toolsets: Optional[List[str]] = None, disabled_toolsets: Optional[List[str]] = None,
    ...
    try:
        ...
        if not skip_tool_search_assembly and ts_cfg.enabled != "off":
            assembly = assemble_tool_defs(filtered_tools, context_length=_resolve_active_context_length(),
                                          config=ts_cfg, sticky_active=sticky_active)
...
def _execute_tool(function_name: str, function_args: Dict[str, Any], original_args: Dict[str, Any], ids: _CallIds,
    ...
    # FORK: the caller's agent ref rides along to registry.dispatch so agent-aware
    # handlers (cross-session messaging, delegate/consult deadline checks) can read
    # it out of **kwargs. Omitted entirely when None so handlers that don't accept
    # it keep working.
    dispatch_kwargs: Dict[str, Any] = {"task_id": ids.task_id, "session_id": ids.session_id}
    if agent is not None:
        dispatch_kwargs["agent"] = agent
