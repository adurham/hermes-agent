...
class TestStaleWatchdogNeverClosesSharedClient:
    ...
    def test_stale_stream_kill_does_not_replace_primary(
        ...
        # Fork: before the first stream event the watchdog applies a cold-start
        # grace window (agent/fork/stream_recovery.effective_stale_timeout,
        # default 600s). Shrink it so the stale kill fires promptly here.
        monkeypatch.setenv("HERMES_STREAM_COLD_START_TIMEOUT", "0.05")
