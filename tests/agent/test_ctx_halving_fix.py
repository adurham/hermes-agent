...
class TestBuildAnthropicKwargsClamping:
    ...
    def test_no_clamping_when_output_ceiling_fits_in_window(self):
        """Opus 4.6 native output ceiling < context window (200K) — no clamping.

        NOTE: this is an upstream test (new in this merge, not present at the
        fork's pre-merge tip) that assumed upstream's own
        _ANTHROPIC_OUTPUT_LIMITS entry for claude-opus-4-6 (128_000). The fork
        deliberately overrides that entry to 16_000 — a long-standing,
        pre-merge customization (see agent/anthropic_adapter.py,
        _ANTHROPIC_OUTPUT_LIMITS) that matches Claude Code 2.1.119's actual
        wire behavior, verified by disassembly of the real client binary
        (`max_tokens: 16000` appears 7x). Since hermes spoofs Claude Code
        identity to use the OAuth token, matching its real max_tokens keeps
        backend scheduling signals consistent. 16_000 still fits well under
        the 200K window, so the "no clamping" behavior this test exists to
        check still holds — only the expected ceiling value changes.
        """
