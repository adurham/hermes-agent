...
class TestRunConversation:
    ...
    def test_length_reasoning_channel_exhausted_skips_continuation(self, agent):
        ...
        reasoning_content but empty content and no tool calls gets ONE
        continuation attempt (a single length-cut with in-progress reasoning
        can legitimately resolve on retry — see
        tests/agent/test_empty_terminal_reasoning_surface.py::test_length_cut_reasoning_is_not_promoted,
        upstream's own invariant for this exact shape). Only a REPEAT of the
        same symptom (this mock always returns the identical exhausted
        response) is treated as genuine thinking-budget exhaustion — the
        real incident this guard exists for was a model stuck re-exhausting
        every retry with zero progress, not a single length-cut."""
        self._setup_agent(agent)
        resp = _mock_response(
            content="",
            finish_reason="length",
        ...
        # One continuation attempt, then exhaustion on the repeat (same mock
        # response both times) — not detected on the very first call.
        assert result["completed"] is False
        assert result["api_calls"] == 2
        assert "reasoning" in result["error"].lower()
        assert "output tokens" in result["error"].lower()
    ...
    def test_length_reasoning_field_exhausted_skips_continuation(self, agent):
        ...
        field (not reasoning_content) — see the sibling test above for why
        this needs one continuation attempt before exhaustion is declared."""
        self._setup_agent(agent)
        resp = _mock_response(
            content=None,
            finish_reason="length",
