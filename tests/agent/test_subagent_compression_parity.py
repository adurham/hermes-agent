...
class TestSubagentCompressionParity:
    ...
    def test_run_conversation_invokes_compress_context_over_threshold(
        ...
        ``_compress_context`` identically for BOTH platforms once real
        provider usage has proven the session is over threshold — the
        direct behavioral disproof of a subagent-specific compression skip.

        NOTE ON THE ROUGH-ESTIMATE PATH (updated 2026-09-14 sync): upstream
        commits 0f4587e336 / e9313f6458 deliberately changed
        ``ContextCompressor.should_defer_preflight_to_real_usage`` so a rough
        (bytes/4) whole-context estimate over threshold, with NO real usage
        recorded yet and the provider not yet proven to omit usage, now
        DEFERS for exactly one request instead of compressing immediately —
        CJK text and reasoning-replay blobs can inflate the rough estimate
        2-3x real usage, and compacting on that noisy signal on literally
        the first turn of a fresh session (before the provider has had a
        chance to report real counts) destroyed detail for nothing. This is
        upstream's considered fix for repeated pointless compaction (a
        different, previously-reported bug), not a subagent-specific
        regression: the deferral is bounded to one request (the overflow
        handler compacts reactively if the guess was right), and it applies
        identically regardless of platform. The old assertion here
        (``_compress_context`` fires on turn 1 purely from an untested rough
        estimate) no longer holds for either platform, so this test now
        establishes real over-threshold usage via a first turn before
        asserting the preflight gate fires on turn 2 — preserving the
        actual invariant under test (platform parity), not the specific
        turn-1-fires-on-a-guess mechanism upstream retired.
        """
        agent = _make_agent(platform, parent_session_id=parent_session_id)
        threshold = agent.context_compressor.threshold_tokens

        agent.client = MagicMock()
        # Turn 1: a small request whose PROVIDER-REPORTED usage is already over
        # threshold (simulates a session that grew via many small tool results,
        # each individually cheap by rough bytes/4 estimate — real usage is the
        # only signal that would catch it). This anchors last_real_prompt_tokens
        # over threshold so should_defer_preflight_to_real_usage() no longer
        # waits on turn 2's rough estimate.
        over_threshold_usage = SimpleNamespace(
            prompt_tokens=threshold + 100, completion_tokens=10,
            total_tokens=threshold + 110,
        )
        first_response = _mock_response()
        first_response.usage = over_threshold_usage
        agent.client.chat.completions.create.return_value = first_response
        ...
        assert agent.context_compressor.last_real_prompt_tokens >= threshold

        # Turn 2: a fresh, comfortably-over-threshold rough estimate. Real usage
        # already proved the session is over threshold, so the preflight gate
        # must compress NOW rather than deferring again.
