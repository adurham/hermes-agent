...
def _make_agent(*, platform="cron"):
    ...
    # A bare MagicMock auto-vivifies attribute access, so unset attributes like
    # _pending_redirect/_pending_steer read as truthy Mock objects instead of
    # the None a real AIAgent starts with. AIAgent.interrupt()'s redirect/steer
    # fold-in logic treats any truthy value here as a real pending correction
    # to fold into the interrupt message via "\n\n".join(...), which crashes
    # on a Mock (not a str). Real agents start with these unset/None.
