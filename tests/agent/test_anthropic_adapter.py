...
class TestBuildAnthropicClient:
    ...
    def test_custom_base_url(self):
        ...
        #
        # Beta order comes from sorted(_ANTHROPIC_NATIVE_ONLY_BETAS) (set
        # iteration order isn't guaranteed) — assert set membership on the
        # comma-split, not a literal joined string. default_headers also
        # carries Authorization: Omit() for api_key-style auth: _new_sdk_client
        # always suppresses whichever of api_key/auth_token isn't the active
        # credential, so a foreign header never leaks to a third-party host.
        with patch("agent.anthropic_adapter._anthropic_sdk") as mock_sdk:
            ...
            assert betas == {
                ...
                "extended-cache-ttl-2025-04-11", "redact-thinking-2026-02-12",
...
class TestBuildAnthropicKwargs:
    ...
    def test_fast_mode_oauth_default_keeps_context_1m_beta(self, monkeypatch):
        ...
        Model is opus-4-8, not opus-4-6: fast mode is an explicit allowlist
        (_FAST_MODE_SUPPORTED_SUBSTRINGS), not a version floor — Opus 4.6 lost
        fast-mode support (requests silently run at standard speed instead of
        erroring), Opus 4.7 hard-400s on the speed param. See
        _supports_fast_mode's docstring.
        """
    ...
    def test_fast_mode_oauth_drop_context_1m_beta_strips_only_1m(self):
        ...
        Model is opus-4-8 — see test_fast_mode_oauth_default_keeps_context_1m_beta
        for why opus-4-6 no longer applies here.
        """
        kwargs = build_anthropic_kwargs(
            model="claude-opus-4-8",
            messages=[{"role": "user", "content": "Hi"}],
    ...
    def test_fast_mode_not_applied_on_opus_46(self):
        """Opus 4.6 lost fast-mode support (2026-09 sync): requests with
        fast_mode=True silently run at standard speed instead of setting
        speed="fast" — see _supports_fast_mode's docstring. Opus 4.7 hard-400s
        on the param instead; only opus-4-8/opus-5 are on the allowlist now."""
        kwargs = build_anthropic_kwargs(
            model="claude-opus-4-6",
            messages=[{"role": "user", "content": "hi"}],
            tools=None,
        ...
        assert kwargs.get("speed") is None
        assert "fast-mode-2026-02-01" not in (kwargs.get("betas") or [])

    def test_fast_mode_still_applied_on_opus_48(self):
        """Regression guard — fast mode must still work on Opus 4.8."""
        kwargs = build_anthropic_kwargs(
            model="claude-opus-4-8",
            messages=[{"role": "user", "content": "hi"}],
            tools=None,
...
class TestToolChoice:
    ...
    def test_specific_tool_choice_oauth_gets_wire_name(self):
        # On the OAuth wire, every tools[] entry is mcp__-prefixed (see the
        # OAuth identity block in build_anthropic_kwargs) — a forced
        # tool_choice must go through the same mapping, or it names a tool
        # that no longer exists on the wire once translation runs.
        kwargs = build_anthropic_kwargs(
            model="claude-sonnet-4-20250514",
            messages=[{"role": "user", "content": "Hi"}],
            tools=self._DUMMY_TOOL,
