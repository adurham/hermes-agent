...
class TestApplyAnthropicCacheControl:
    ...
    def test_caller_list_not_mutated_and_unmarked_msgs_shared(self):
        ...
        # reserve_tools_breakpoint=False: this test pins the legacy 3-message
        # layout; the default reserves one breakpoint for tools[] and marks
        # only the last 2 non-system messages.
        result = apply_anthropic_cache_control(msgs, cache_ttl="5m", reserve_tools_breakpoint=False)
    ...
    def test_output_equivalent_to_full_deepcopy_impl(self):
        ...
        for native in (True, False):
            for ttl in ("5m", "1h"):
                # reserve_tools_breakpoint=False matches the reference impl's
                # legacy system-and-3 layout (no tools[] breakpoint reserved).
    ...
    def test_static_system_prefix_gets_its_own_marker(self):
        ...
        # reserve_tools_breakpoint=False: the static prefix consumes 2 system
        # breakpoints, leaving 2 for the trailing messages this test asserts
        # on; the default would reserve one of those for tools[].
