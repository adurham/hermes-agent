...
class TestAgentLoopSourceHasNoneTypeCarveOut:
    ...
    Upstream's Sep 2026 turn-phase refactor split the classifier out of
    ``agent/conversation_loop.py`` (inline in the main loop pre-merge) into
    ``agent/turn_api_error.py::_is_local_validation_error`` (see
    ``agent/turn_api_error.py``'s own module history) — ``conversation_loop.py``
    now only imports ``handle_api_error`` from it. Check the module that
    actually owns the predicate today.
    """

    def test_conversation_loop_excludes_nonetype_not_iterable_from_local_validation(self):
        import inspect
        from agent import turn_api_error
        src = inspect.getsource(turn_api_error)
        ...
        assert "nonetype" in src.lower() and "not iterable" in src.lower(), (
            "agent/turn_api_error.py must carve out 'NoneType is not iterable' "
...
class TestAgentLoopSourceHasToolJSONCarveOut:
    ...
    Same relocation as ``TestAgentLoopSourceHasNoneTypeCarveOut`` above: the
    classifier now lives in ``agent/turn_api_error.py``, not
    ...
    def test_conversation_loop_excludes_tool_json_parse_error_from_local_validation(self):
        ...
        assert "unable to parse tool parameter json" in src.lower(), (
            "agent/turn_api_error.py must carve out the SDK's 'Unable to "
