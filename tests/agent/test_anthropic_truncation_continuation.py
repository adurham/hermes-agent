...
class TestTruncatedToolCallContinuationParity:
    ...
    def _loop_source(self) -> str:
        # NOTE: this fork already extracts truncation recovery out of
        # agent/conversation_loop.py into agent/turn_truncation.py (a
        # pre-merge refactor of this hot path, unrelated to the upstream
        # sync) -- the tool-call retry-budget guard and the ephemeral-budget
        # boost now live in `_retry_truncated_tool_call` there, not inline
        # in conversation_loop.py. Read that module's source instead.
        import inspect
        import agent.turn_truncation as tt
        return inspect.getsource(tt)
    ...
    def test_tool_call_path_boosts_ephemeral_output_budget(self):
        ...
        tc_branch = inspect.getsource(_retry_truncated_tool_call)
