...
class TestAnthropicStreamCallbacks:
    ...
    def test_anthropic_malformed_tool_json_retries_with_buffered_tool_input(self):
        ...
        # Force the plain (non-beta) `.messages.stream` code path — see the comment on the
        # first occurrence of this line above (test_anthropic_stream_refreshes_activity_on_every_event):
        # a bare MagicMock() auto-vivifies `.beta.messages` as a truthy mock, so without this the
        # production code prefers `.beta.messages.stream(...)` and `_stream` here never fires.
        agent._anthropic_client.beta = None
    ...
    def test_anthropic_partial_tool_names_do_not_survive_into_next_attempt(self):
        ...
        # first occurrence of this line above (test_anthropic_stream_refreshes_activity_on_every_event):
        # a bare MagicMock() auto-vivifies `.beta.messages` as a truthy mock, so without this the
        # production code prefers `.beta.messages.stream(...)` and the `attempts` side_effect
        # below never fires.
        agent._anthropic_client.beta = None
