...
def _make_agent(monkeypatch):
    ...
    # Real signature is (tool_name, function_args, function_result, *, failed, tool_call_id).
    # The previous 2-positional stub silently returned `function_args` (a dict) in place of
    # `function_result`; harmless upstream (no str-only consumer downstream) but this fork's
    # tools/content_filter_scrub.scrub_trigger_patterns (called unconditionally from
    # maybe_persist_tool_result) requires a string and raises TypeError on a dict.
    stub._append_guardrail_observation = lambda name, args, result, *a, **kw: result
