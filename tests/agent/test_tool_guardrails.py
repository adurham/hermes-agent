...
def test_default_config_has_hard_stops_enabled():
    # FORK: hard_stop_enabled defaults True (upstream ships False + non-interactive
    # escalation only). See FORK.md 2026-07-07 'tool-call loop guardrails now block
    # by default'. Upstream's non_interactive_hard_stop_enabled is kept and is a
    # no-op while hard_stop_enabled is True, so its assertion is retained.
...
def test_gateway_platform_defaults_to_hard_stop_without_changing_interactive_defaults():
    # FORK: hard_stop_enabled defaults True everywhere, interactive platforms included
    # (upstream ships False for interactive platforms + auto-on for non-interactive only).
    # See FORK.md 2026-07-07 'tool-call loop guardrails now block by default'.
    interactive_configs = [
...
def test_non_interactive_hard_stop_can_be_disabled_explicitly():
    # FORK: hard_stop_enabled's base default is True regardless of
    # non_interactive_hard_stop_enabled, so disabling the latter alone doesn't flip
    # hard_stop_enabled to False on this fork (it's a no-op escalation flag here).
    # See FORK.md 2026-07-07. Explicit hard_stop_enabled=False is the real opt-out.
    cfg = ToolCallGuardrailConfig.from_mapping(
...
def test_supervised_task_platforms_keep_warning_only_default():
    # FORK: hard_stop_enabled defaults True on every platform, "supervised" or not
    # (upstream keeps supervised/interactive platforms warn-only by default). See
    # FORK.md 2026-07-07 'tool-call loop guardrails now block by default'.
