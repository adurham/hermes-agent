...
def _make_anthropic_agent(**kwargs):
    ...
    # Fork-specific: _call_anthropic prefers request_client.beta.messages when
    # it is present (Claude-Code-mimicry beta kwargs; see
    # test_anthropic_stream_beta_routing.py). A bare MagicMock() auto-vivifies
    # `.beta.messages` as a truthy attribute, so without this the fork's beta
    # check silently redirects .stream() calls away from the `.messages.stream`
    # mock set up below and onto an unconfigured MagicMock, decoupling the
    # test's assertions from what actually ran. Force the plain-.messages path
    # (upstream has no beta-preference branch at all) so this test exercises
    # the tool-call-drop logic it's named for, not beta routing (covered
    # separately).
