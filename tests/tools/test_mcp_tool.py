...
class TestDiscoverAndRegister:
    ...
    def test_same_server_normalization_collision_skips_all_ambiguous_tools(self, caplog):
        ...
        assert registry.get_entry("srv_read_file") is None
        assert registry.get_entry("srv_safe_tool") is not None
        assert any(
...
class TestDiscoveryFailedCount:
    ...
    def test_failed_server_increments_failed_count(self):
        ...
        with patch("tools.mcp_tool_config._load_mcp_config", return_value=fake_config), \
             ...
             patch("tools.mcp_tool_registration._existing_tool_names", return_value=["good_server_tool_a"]):
            _ensure_mcp_loop()
...
class TestMCPBuiltinCollisionGuard:
    ...
    def test_mcp_tool_rejected_when_collision_is_another_mcp(self):
        ...
        # (Fork naming convention: `{server}_{tool}`, no `mcp__` prefix — see
        # FORK.md / is_mcp_tool_parallel_safe.)
        assert "srv_do_thing" not in registered
        entry = mock_registry.get_entry("srv_do_thing")
        assert entry is not None
        assert entry.toolset == "mcp-old"
        assert entry.schema["description"] == "From another MCP server"
        assert mock_registry.get_toolset_for_tool("srv_do_thing") == "mcp-old"
...
class TestRegisterMcpServers:
    ...
    def test_connects_new_servers(self):
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), \
             patch("tools.mcp_tool_discovery._discover_and_register_server", side_effect=fake_register), \
             patch("tools.mcp_tool_registration._existing_tool_names", return_value=["my_server_tool1"]):
            _ensure_mcp_loop()
