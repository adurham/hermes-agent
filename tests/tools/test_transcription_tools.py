...
class TestTranscribeLocalExtended:
    ...
    def test_config_device_and_compute_type_passed_to_whisper(self, tmp_path):
        ...
        # _should_force_faster_whisper_cpu() pins cpu/int8 on Apple Silicon
        # hosts before config is consulted; disable it so this test checks
        # the config forwarding contract regardless of the host machine.
        with patch("tools.transcription_tools._HAS_FASTER_WHISPER", True), \
             patch("faster_whisper.WhisperModel", mock_whisper_cls), \
             patch("tools.transcription_tools._local_model", None), \
             patch("tools.transcription_tools._local_model_name", None), \
             patch("tools.transcription_tools._should_force_faster_whisper_cpu",
                   return_value=False), \
