...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    ...
    # Sorted: ["consult", "kanban", "memory", "project"]. `consult` (fork
    # feature — "get a second opinion" tool, see hermes_cli/config.py's
    # AUXILIARY_DEFAULTS["consult"] comment) and `kanban` are both
    # auto-recovered by _get_platform_tools as non-configurable members of
    # the "cli" platform's default composite (`hermes-cli`); `project` is
    # GUI-only, folded in by _load_enabled_toolsets.
    assert server._load_enabled_toolsets() == ["consult", "kanban", "memory", "project"]
