...
def test_api_key_providers_expose_a_credential_env_var():
    ...
    ``custom`` bring-your-own-endpoint pseudo-provider, and ``exo``.
    ``custom``/``exo`` are configured inline via the local-endpoint flow
    (base_url set by the user) rather than a fixed credential env var;
    ``exo``'s profile declares ``env_vars=()`` explicitly: "No fixed API key —
    uses the cluster's own auth" (plugins/model-providers/exo/__init__.py).
    """
    exempt = {"custom", "exo"}
