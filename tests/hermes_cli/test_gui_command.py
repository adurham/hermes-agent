...
def test_relaunch_fixup_signs_with_entitlements_and_hardened_runtime(tmp_path, monkeypatch):
    ...
    # The inside-out signer requires the inherit plist too (helper apps);
    # missing either one deliberately falls back to the bare ad-hoc path.
    (desktop_dir / "electron" / "entitlements.mac.inherit.plist").write_text("<plist/>")
    ...
    # Upstream's real-signature precheck (`codesign -dv`) may run first;
    # locate the xattr clear. The inside-out signer signs the main bundle
    # last, with the app's own entitlements and the hardened runtime.
    xattr_idx = calls.index(["xattr", "-cr", str(app)])
    sign_cmds = [
        c for c in calls[xattr_idx + 1:]
        if c and c[0] == "/usr/bin/codesign" and "--sign" in c
    ]
    assert sign_cmds, f"no codesign --sign call recorded after xattr: {calls}"
    main_sign = next(c for c in sign_cmds if c[-1] == str(app))
    assert "--force" in main_sign
    assert main_sign[main_sign.index("--sign") + 1] == "-"
    assert "--options" in main_sign and "runtime" in main_sign
    assert "--entitlements" in main_sign
    assert str(entitlements) in main_sign
...
def test_relaunch_fixup_falls_back_to_bare_adhoc_without_entitlements_file(tmp_path, monkeypatch):
    ...
    # Upstream's real-signature precheck (`codesign -dv`) may run first;
    # locate the xattr clear and assert the re-sign follows it.
    xattr_idx = calls.index(["xattr", "-cr", str(app)])
    sign_cmd = calls[xattr_idx + 1]
