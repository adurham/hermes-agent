...
class TestWebServerEndpoints:
    ...
    def test_get_session_messages_follows_compression_tip(self):
        ...
        try:
            ...
            # Empty the compressed root's transcript directly: replace_messages
            # now refuses compression-closed sessions (CompressionSessionClosedError),
            # and this is fixture setup, not the behavior under test.
            db._conn.execute("DELETE FROM messages WHERE session_id = ?", ("desktop-root",))
            db._conn.execute(
    ...
    def test_audio_transcription_endpoint(self, monkeypatch):
        ...
        def fake_transcribe_audio(path, model=None, **kwargs):
            captured["path"] = path
            return {
                "success": True,
    ...
    def test_speak_text_returns_base64_data_url(self, monkeypatch, tmp_path):
        ...
        def fake_tts(text, provider=None, **kwargs):
            return json.dumps({
                "success": True,
