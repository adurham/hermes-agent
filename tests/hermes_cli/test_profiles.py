...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_history_artifacts(self, profile_env):
        ...
        # Files at the source root are excluded outright.
        for history in (
            "state.db", "state.db-wal", "state.db-shm",
            "backups", "state-snapshots", "checkpoints",
        ):
            assert not (profile_dir / history).exists(), history
        # `sessions/` and `cron/` are excluded as CONTENT but deliberately re-created as
        # empty skeletons so the clone can run (upstream 48465c3933: "--clone-all no longer
        # copies cron jobs"; the same re-mkdir covers sessions). Asserting the dir is absent
        # would contradict that — the contract is that no source history came across.
        for skeleton in ("sessions", "cron"):
            d = profile_dir / skeleton
            assert d.is_dir(), f"{skeleton}/ should exist as an empty skeleton"
            assert not any(d.iterdir()), f"{skeleton}/ should carry no source history"
        assert not (profile_dir / "sessions" / "20260101_old").exists()
        assert (profile_dir / "config.yaml").read_text() == "model: gpt-4"
