...
def test_healthy_snapshot_carries_no_error_keys():
    ...
    snapshot = server._inflight_snapshot(session)
    # FORK: the snapshot also carries the turn's real start time so a
    # reconnect doesn't reset the frontend's "thinking" timer; drop it
    # before asserting no ERROR keys leak into a healthy snapshot.
    assert isinstance(snapshot.pop("started_at", None), (int, float, type(None)))
