...
def _bootstrap_profile_dir(profile_dir: Path, source_dir: Optional[Path], link: bool = False,
    ...
    With ``link=True`` the shareable state (skills, plugins, both memory tiers) is symlinked
    at the source instead of copied — see ``create_profile``'s ``link`` parameter.
    ...
    # --link points profile_dir/skills AT the source's skills/ dir instead of copying it: both
    # profiles read and write the exact same on-disk tree, so a skill created, edited, or
    # curator-pruned from either profile is instantly visible in the other with no re-sync step.
    ...
    # --link also shares the plugins/ directory the same way, when the source profile has one.
    # Plugins are typically empty/absent (installed at the shared ~/.hermes level in most setups)
    # but honor the same link semantics as skills for consistency.
    if link:
    ...
    # Clone installed petdex mascots from the source profile. Without this, config.yaml's
    # display.pet.slug still points at a pet the new profile never downloaded — the sprite
    # assets live in pets/<slug>/ (spritesheet.webp + pet.json), not in config.yaml.
    ...
    if link:
        # --link shares BOTH memory tiers with the source profile: the hot-tier memories/
        # directory (MEMORY.md, USER.md — the files _CLONE_SUBDIR_FILES would otherwise copy
        # once) and the warm-tier memory_store.db (+ -wal/-shm siblings, which sqlite recreates
        # next to whatever path it's opened through). This is for profile pairs that are the
        # same "self" running under a different model/provider, where facts learned under one
        # provider should be visible under the other immediately, not copied-then-drifted.
...
def create_profile(
    ...
    link: bool = False, no_alias: bool = False, no_skills: bool = False, description: Optional[str] = None,
    ...
    ``link`` shares skills, plugins, and memory (hot-tier MEMORY.md/USER.md plus the warm-tier
    memory_store.db) with the source via symlinks instead of copying: edits/learns in either
    profile are immediately visible in both, forever, with no re-sync step. It implies
    ``clone_config`` (config.yaml/.env/SOUL.md stay independent *copies* so per-profile
    model/provider routing can diverge) and is for "same brain, different model" profile pairs,
    not fully isolated personas; mutually exclusive with ``clone_all`` and ``no_skills``.
    ...
    try:
        if clone_all and source_dir:
            ...
            _bootstrap_profile_dir(staging, source_dir, link=link, sync_imports=sync_imports)
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    # Block deletion outright when a live systemd unit (any scope, any name — see
    # _find_systemd_units_referencing_profile) depends on this profile. Without this, delete
    # proceeds, the unit's next restart hits "EnvironmentFile: No such file or directory" and
    # crash-loops (or, for a unit that hadn't restarted yet, silently keeps running against a
    # directory that no longer exists until its next restart kills it) — with zero warning to
    # whoever ran the delete (2026-09-08 incident: this exact path deleted the profile backing
    # the dashboard process serving the delete request itself). ``yes=True`` (the dashboard's
    # confirm-then-delete flow) does NOT bypass this — a typed confirmation of profile name is
    # not informed consent to breaking an unrelated system service the user may not know exists.
