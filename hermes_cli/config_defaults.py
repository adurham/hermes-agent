...
DEFAULT_CONFIG = {
    ...
    "display": {
        ...
        # FORK: which key interrupts a running agent in the classic CLI:
    ...
    "skills": {
        ...
        # Fork-only: lazy-load the skills index. When True, the system prompt
        # skips the bulky ``<available_skills>`` block (one entry per skill —
        # names + descriptions for every skill in every external_dirs source).
        # The model uses the existing ``skills_list`` tool to discover skills
