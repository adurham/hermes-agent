...
def _plugin_web_search_providers() -> list[dict]:
    ...
    for provider in providers:
        ...
        # ``web.provider_tier.<name>`` on selection). Each row is built via
        # _plugin_provider_row so the fork's auth-gate signals
        # (requires_nous_auth / managed_nous_feature / _is_available) survive
        # on every variant, not just the base schema.
        schemas = [schema] + [
            v for v in (schema.get("variants") or []) if isinstance(v, dict)
        ]
        for entry in schemas:
            row = _plugin_provider_row(provider, entry)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    ...
    print(color("  Guide: https://github.com/adurham/hermes-agent/blob/main/website/docs/user-guide/features/tools.md", Colors.DIM))
    print()
