...
class CLIModalMixin:
    ...
    def _clarify_callback(self, question, choices, multi_select=False, questions=None):
        ...
        # FORK: bell + macOS notification — clarify questions can sit on screen for a long
        # time before the user notices (ported from the pre-decomposition cli.py callback).
    ...
    def _clarify_callback_batch(self, questions):
        ...
        self._clarify_deadline = None if timeout <= 0 else _time.monotonic() + timeout
        self._ring_bell(prompt=True, context="clarify")
        # FORK: first question text (plus a count suffix) as the notification summary —
        # mirrors the single-question path's construction.
    ...
    def _sudo_password_callback(self) -> str:
        ...
        # FORK: ported from the pre-decomposition callback.
    ...
    def _approval_callback(self, command: str, description: str,
        ...
        with self._approval_lock:
            # FORK: resolve through tools.approval_context._get_approval_timeout() rather than
            # reading CLI_CONFIG directly — it is the single owner of the resolved approval
            # window (config default, clamping against agent.deadline.MAX_SAFE_TIMEOUT_S, and
            # any per-call override). The direct read silently ignored all of that, so a
            # configured approvals.timeout was not honored. See
            # tests/hermes_cli/test_approval_attention_signals.py::TestApprovalTimeoutWiring.
            try:
            ...
            # FORK: bell + native notification so the user notices the prompt even when
            # they're in another window or SSH'd in from elsewhere (ported from the
            # pre-decomposition cli.py callback). Long descriptions are trimmed.
