...
# Fork guard helpers: _apply_profile_override() runs at MODULE IMPORT TIME, but "this module got
# imported" and "the hermes CLI was invoked" are different events. pytest's own plugin-activation
# flag (`-p randomly`, pytest-randomly) is shaped exactly like a profile name and slipped past the
# _PROFILE_NAME_RE guard below, sys.exit(1)-ing ("Profile 'randomly' does not exist") the instant
# any test file importing hermes_cli.main ran under `pytest -p randomly`. Gate on sys.argv[0]
# IDENTITY rather than an env-var "are we under pytest" proxy: that proxy broke
# tests/hermes_cli/test_apply_profile_override.py, which calls this function directly with a
# test-constructed argv ["hermes", "-p", "elias", ...] and legitimately wants the override to run.
_HERMES_SCRIPT_NAMES = {"hermes", "hermes-agent", "hermes-acp"}


def _looks_like_hermes_invocation(argv0: str = "") -> bool:
    """True if we are running under a real Hermes CLI entry point.

    Two callers with different needs share this predicate:
      * ``_scan_profile_flag`` guards its loud rejection with the no-arg form
        (``_looks_like_hermes_invocation()``) — pytest's own ``-p no:xdist`` reaches
        that scan through ``sys.argv`` at import time, so it must stay a silent skip,
        while a real ``hermes -p 'Work Bot'`` must fail loudly.
      * the fork's argv0 preflight passes ``sys.argv[0]`` explicitly.

    When ``argv0`` is omitted, ``sys.argv[0]`` is used.
    ...
    """
    argv0 = argv0 or (sys.argv[0] if sys.argv else "")
    # Test-runner guard (fork): never treat pytest's argv as a hermes invocation.
    if "pytest" in argv0:
        return False
    if not argv0:
        return False
...
def _apply_profile_override() -> None:
    ...
    # Fork: only act on a real Hermes CLI argv0 — see _looks_like_hermes_invocation.
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        "acp", "agents", "approvals", "auth", "backup", "bundles", "checkpoints", "claw", "codex-runtime", "completion",
        "computer-use",
        ...
        "mcp-gateway",
        "model", "monitoring", "pairing", "pause", "peer", "pets", "plugins", "portal", "postinstall", "profile",
        "project", "proxy",
        "prompt-size",
        "resume",
        "send", "sessions", "setup",
        "skin", "skills", "slack", "status", "submit", "sync", "tools", "uninstall", "update",
        "usage", "vault",
...
def _build_cli_parser():
    ...
    # =========================================================================
    # mcp-gateway command — expose the remote gateway as MCP tools (fork)
    # =========================================================================
    ...
    # Fork: keep `hermes agents` registered. Upstream v2026.9.14 imports
    # build_agents_parser but never calls it, while "agents" is still in the
    # known-subcommand list below — preserving the fork's call keeps the
    # command reachable (and the import used).
