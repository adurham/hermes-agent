...
def _disabled_toolset_names() -> set[str]:
    ...
    FORK helper, kept through the Sep 2026 doctor decomposition. Keeps the doctor's "Tool
    Availability" section (a raw dependency/capability probe, independent of whether a toolset
    is actually wired into the live agent) from nagging about toolsets the user intentionally
    turned off via config. Fails open to an empty set on any config read error — better to show
    a stray warning than silently hide a real one.
    """
    try:
