...
def _profile_create(args):
    ...
    # FORK: --link shares skills/plugins/both memory tiers with the source via symlinks.
    # create_profile() has owned this behavior since the feature landed; the CLI flag was
    # registered but never forwarded here, so `--link` silently created a plain copy.
    ...
    try:
        profile_dir = create_profile(
            ...
            link=link, no_alias=no_alias, no_skills=no_skills, description=getattr(args, "description", None),
