...
def _run_single_query_mode(cli, query, image, quiet, oneshot, stream_json: bool = False):
    ...
    from cli import _SeededQueryMessage, _collect_kanban_task_images, _collect_query_images, _configure_quiet_agent, _finalize_single_query, _fold_curator_cost_before_exit, _route_single_query_images, _run_kanban_goal_loop_chat, _run_memory_confirm_before_exit, _run_quiet_single_query, _should_seed_interactive, _single_query_exit_code
    ...
    try:
        ...
        if os.environ.get("HERMES_KANBAN_GOAL_MODE") == "1":
            try:
                ...
                logger.debug("kanban goal loop failed: %s", _goal_exc)
        # FORK order: fold the memory-confirm LLM spend into the session total BEFORE the
        # summary reads it (see cli._run_memory_confirm_before_exit's docstring and
        # tests/hermes_cli/test_exit_summary_before_cleanup_ordering.py). The stdin-unavailable
        # and interactive-exit sites do the same via _tui_shutdown.
