...
class SubagentMonitor:
    ...
    def refresh(self, now=None):
        ...
        signature = json.dumps([ordered, processes, goal, queued], sort_keys=True, default=str)
    ...
    def dock_text(self, *, columns, rows):
        ...
        if self.entries:
            ...
            for row in self.entries[:agent_count]:
                ...
                activity_budget = max(0, columns - get_cwidth(prefix) - get_cwidth(elapsed) - 12)
                activity = f"{elapsed} · {row_activity(row, width=activity_budget)}"
