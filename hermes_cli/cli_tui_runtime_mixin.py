...
class CLITuiRuntimeMixin:
    ...
    def _tui_shutdown(self):
        ...
        from cli import _DIM, _RST, _cprint, _fold_curator_cost_before_exit, _invoke_interrupted_session_end, _run_cleanup, _run_memory_confirm_before_exit, set_approval_callback, set_secret_capture_callback, set_sudo_password_callback
        self._should_exit = True
        self._pet_stop_anim()
        ...
        # FORK order (see tests/hermes_cli/test_exit_summary_before_cleanup_ordering.py):
        # the exit summary MUST print before _run_cleanup(). Cleanup can block for tens of
        # seconds (Phase-2 memory-confirm LLM call, MCP teardown) and the exit watchdog
        # force-exits the process if it overruns, which would silently swallow the cost
        # report + --resume hint. Both helpers also fold their own LLM spend into
        # session_estimated_cost_usd first, so they must run before the summary is rendered.
