...
def detect_provider_for_model(
    ...
    # Before the static/OpenRouter catalogs (but AFTER upstream's two
    # stay-on-the-current-provider guards above), honor a model the user
    # explicitly declared under ``providers.<name>.models`` (e.g. an exo
    # cluster model, an ollama-cloud tag). Without this, a bare ``/model
    # <name>`` for such a model falls through to None and switch_model keeps
    # the CURRENT provider — so a cold-start session on Anthropic that
    # switches to an exo model gets stamped provider='anthropic'. That
    # mislabel then cascades: every auxiliary task (title_generation,
    # vision, …) resolves against the anthropic block and a Claude model
    # name gets shipped to the exo endpoint → recurring 404. Consulting the
    # user's own provider→models map fixes the whole class and makes bare
    # ``/model`` match the explicit ``--provider`` form.
...
def _credential_fingerprint(provider: str) -> str:
    ...
    # FORK — Config-level endpoint override (config.yaml ``model.base_url`` /
