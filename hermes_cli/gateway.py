...
def _run_systemctl(args: list[str], *, system: bool = False, **kwargs) -> subprocess.CompletedProcess:
    ...
    FORK: system-scope MUTATING calls (anything not in ``_SYSTEMCTL_READONLY_VERBS``) from a
    non-root user transparently try ``sudo -n`` first when — and only when — a passwordless
    grant exists for that exact action+unit. Actions with no grant fall straight through to the
    plain call. Read-only verbs never probe/sudo: querying system-unit state needs no privilege,
...
def _require_root_for_system_service(action: str, *, unit: str | None = None) -> None:
    ...
    FORK: root always qualifies. When ``unit`` is given, a non-root caller also qualifies if a
    passwordless ``sudo -n systemctl <action> <unit>`` grant exists — the same check
    ``_run_systemctl`` uses to decide whether to prefix its call with ``sudo -n``, so a caller
    passing this gate is guaranteed the mutating calls after it succeed rather than merely
    getting further before failing. ``unit`` is omitted for actions nothing is ever scoped to
    grant (install/uninstall write unit files as root); those stay strictly root-only.
    """
    if os.geteuid() == 0:  # windows-footgun: ok — POSIX systemd helper, never invoked on Windows
        return
    if unit and _can_passwordless_sudo_systemctl(_sudo_normalized_systemctl_args([action, unit])):
        return
...
def _systemd_scope_preamble(
    ...
    FORK: ``require_root=False`` skips only the upfront system-scope root gate, for callers whose
    healthy path needs no privilege (systemd_restart's SIGUSR1 drain) and which gate each actual
    systemctl mutation individually against the action it runs.
    """
...
def systemd_restart(system: bool = False):
    # FORK: no upfront root/sudo gate on system scope. The graceful SIGUSR1 self-restart below
    # needs zero privilege (same-uid signal) and is the common healthy path; each actual
    # systemctl mutation gates itself against the specific action it runs, inside
    # _systemd_reset_and_run. A non-root caller with a restart-only passwordless sudoers grant
    # therefore restarts cleanly instead of being told "requires root" for a path not needing it.
    system = _systemd_scope_preamble("restart", system, preflight_user=True, require_root=False)
...
def _systemd_reset_and_run(action: str, *, system: bool, previous_pid) -> None:
    ...
    FORK: this is the single chokepoint for mutating systemctl calls on the restart path, so the
    system-scope privilege gate lives here and is keyed to the ACTION actually about to run. A
    sudoers grant scoped to ``restart`` only (the common deployment) correctly does NOT cover the
    rare ``start`` sub-branch, which then raises a clear "requires root" instead of a silent
    CalledProcessError. Gating here (rather than upfront in systemd_restart) is what lets the
    zero-privilege SIGUSR1 graceful path run for a non-root caller.
    """
    svc = get_service_name()
    if system:
        _require_root_for_system_service(action, unit=svc)
    _run_systemctl(["reset-failed", svc], system=system, check=False, timeout=30)
    try:
