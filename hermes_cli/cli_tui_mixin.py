...
class CLITuiMixin:
    ...
    def _build_tui_layout_children(
        ...
        *,
        ...
        ``reasoning_picker_widget`` and ``todo_board_widget`` are fork additions, ordered the same
        way the other panels are: built elsewhere and passed in, so an absent one is simply
        filtered out. ``_subagent_dock_widget`` follows the same "built elsewhere, hung off
        ``self``" pattern the stash panel and pet widget use: ``install_dock`` populates it before
        the layout is built, and a direct call on a CLI that never built a layout filters the
        missing one out.
        """
        ordered = [
            Window(height=0),
    ...
    def _tui_build_key_bindings(self):
        ...
        _normal_input = Condition(
            ...
            and not getattr(self, "_reasoning_picker_state", None))
