...
PLATFORM_SUPPORT_DOCS_URL = "https://github.com/adurham/hermes-agent/blob/main/website/docs/getting-started/platform-support.md"
...
def _coerce_config_value(key: str, value: str, defaults: dict):
    ...
    Order of resolution (type-directed by the key's DEFAULT_CONFIG value):
      1. If the key is STRING-typed in DEFAULT_CONFIG, the raw string is
         returned unchanged — no bool/JSON coercion attempted. This must run
         BEFORE JSON/bool parsing: enum members such as ``approvals.mode:
         "off"`` must never be silently coerced to the YAML boolean ``False``
         (previously ``hermes config set approvals.mode off`` did exactly
         that, corrupting an enum-valued field).
      2. Explicit JSON (``[...]`` / ``{...}``) is parsed as-is — lets callers
         pass lists/dicts unambiguously, e.g. ``'["a","b"]'``.
      3. If the key is list-typed in DEFAULT_CONFIG, split a comma-separated
      ...
      4. Otherwise fall back to scalar coercion (bool / int / float / str).
    ...
    if isinstance(default_at_key, str):
        return value
    ...
    if low in {"false", "no", "off"}:
        return False
    if low in {"null", "none", "~"}:
        # YAML null / "off" state. Many DEFAULT_CONFIG leaves default to None
        # and are documented as "null/absent = off"; without this,
        # ``config set X null`` stored the truthy string "null" and the
        # feature could never be cleared via set (CFG-05).
        return None
    # int()/float() handle signs, surrounding whitespace, and underscores --
    # unlike the old ``value.isdigit()`` which rejected "-5" and " 5 " and
    # silently stored them as strings on int-typed keys (CFG-02).
    as_int = _coerce_int(stripped)
    if as_int is not None:
        return as_int
    as_float = _coerce_float(stripped)
...
def convert_auxiliary_to_provider_first(aux: dict) -> dict:
    ...
    # Convertibility guard: provider-first has no "auto" provider block, so a
    # config whose tasks carry ONLY auto/empty providers (no explicit provider
    # pin, no fallbacks) has nothing expressible in the new shape. Converting
    # it would silently drop the task's model pin (the resolver reads
    # task-first natively, so leaving it is behavior-preserving; rewriting it
    # to ``{}`` is not).
    def _task_is_convertible(tcfg: dict) -> bool:
        prov = str(tcfg.get("provider", "")).strip()
        if prov and prov != "auto":
            return True
        fbm = tcfg.get("fallback_models")
        if isinstance(fbm, dict) and any(
            str(p).strip() and str(m).strip() for p, m in fbm.items()
        ):
            return True
        return bool(str(tcfg.get("fallback_model", "")).strip())

    if not any(_task_is_convertible(aux[t]) for t in task_keys_present):
        return aux
    ...
    for task in task_keys_present:
        ...
        if prov and prov != "auto":
            ...
            # Explicit model on an auto/empty provider: provider-first has no
            # "auto" block, so carry the pin through the shared ``defaults``
            # block (the resolver merges defaults first, then block/task
            # overrides) instead of silently dropping it.
            defaults_block.setdefault(task, {})["model"] = model
        ...
        if settings:
            defaults_block.setdefault(task, {}).update(settings)
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    # String-typed enum keys (e.g. approvals.mode) are preserved verbatim —
    # see _coerce_config_value's docstring for why this ordering matters.
    coerced = _coerce_config_value(key, value, DEFAULT_CONFIG)

    # _coerce_config_value only attempts a single-line JSON parse
    # (``stripped[:1] in ("[", "{")``). When that's not what the value looks
    # like but it still plausibly encodes a YAML list/mapping — including a
    # multi-line block, e.g.
    #   hermes config set custom_providers '- name: foo
    #     base_url: https://...'
    # — fall back to a full yaml.safe_load so the setting doesn't silently
    # land as a raw STRING that every isinstance(..., list)-gated reader
    # (``_get_platform_tools``, ``_get_enabled_set``, ...) then ignores.
    # Only runs when _coerce_config_value left the value as the original
    # string (i.e. none of its list/bool/int/float paths fired) and
    # _looks_structured_value's conservative heuristic flags it — a plain
    # scalar like '-5' or '--flag' never reaches the YAML parser.
    if coerced == value and not isinstance(_default_value_for_key(key), str) and _looks_structured_value(value):
        try:
