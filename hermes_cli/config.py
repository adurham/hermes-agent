...
import tempfile
import threading
import time  # noqa: F401  (re-exported; historical hermes_cli.config.time consumers)
import unicodedata
...
# Fork-only: per-model ``max_tokens`` default from ``custom_providers``. Sibling of upstream
# ``get_custom_provider_context_length`` (which moved to hermes_cli/config_providers.py in the
# v2026.9.14 decomposition); this one has no upstream counterpart, so it stays here.
...
def _validate_config_key(key: str) -> tuple[bool, Optional[str]]:
    ...
    for seg in segments[1:]:
        ...
        if not isinstance(node, dict) or not node:
            # We hit a scalar leaf or an EMPTY dict before consuming the user's
            # full path. The scalar leaf accepts because set_config_value's
            # coercion will replace it with a dict, matching pre-existing
            # behavior; an empty dict is a free-form mapping section
            # (``compression.model_thresholds.<model>``,
            # ``terminal.docker_env.<VAR>``, ``lsp.servers.<lang>``) whose keys
            # are user-chosen, so nothing under it can be a typo.
            return True, None
        if seg not in node:
...
def _write_user_config(config_path: Path, user_config: Dict[str, Any]) -> None:
    ...
    atomic_config_write(
        ...
        extra_content_on_create=_commented_sections_for_save(user_config))
